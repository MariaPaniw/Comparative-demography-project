...
hist(deltaR.h1-deltaR.h1.V2)# V2 should be smaller, so the difference should be largely positive, and it is for the most part  
plot<-hist(deltaR.c-deltaR.c.V2)# even clearer for the other shrub
ggsave("Cistus.Rain.png", plot, dpi = 300)

# sensitivities for all vital rates (no cov vs cov)
# Halimium
df.h1=data.frame(study.doi="https://doi.org/10.1098/rspb.2022.1494",
                 ...
                 stage.age="all",
                 vital.rates="all",
                 sens.no.cov=c(deltaR.h1,deltaIntra.h1,deltaInter.h1),
                 sens.cov=c(deltaR.h1.V2,deltaIntra.h1.V2,deltaInter.h1.V2))

# Cistus
df.c=data.frame(study.doi="https://doi.org/10.1098/rspb.2022.1494",
                 year.of.publication="2023",
                 group="Plants",
                 ...
                 stage.age="all",
                 vital.rates="all",
                 sens.no.cov=c(deltaR.c,deltaIntra.c,deltaInter.c),
                 sens.cov=c(deltaR.c.V2,deltaIntra.c.V2,deltaInter.c.V2))

# merge
df1=rbind(df.h1,df.c)

# save
write.csv(df1,"sens.shrubs.MCMC.csv",row.names = F)

#### PERTUBRATIONS PER VITAL RATE --------
n.stage=3
...
# sensitivities for all vital rates (no cov vs cov)
# Halimium
df.h2=data.frame(study.doi="https://doi.org/10.1098/rspb.2022.1494",
                 year.of.publication="2023",
                 group="Plants",
                 species="Halimium halimifolium",
                 continent="Europe",
                 ...
                 sens=c(deltaR.h1.survSap,deltaR.h1.survM,
                        deltaIntra.h1.survSap,
                        deltaInter.h1.survM))
# Cistus
df.c2=data.frame(study.doi="https://doi.org/10.1098/rspb.2022.1494",
                 year.of.publication="2023",
                 group="Plants",
                 species="Cistus libanotis",
                 continent="Europe",
                 ...
                 stage.age="adult",
                 vital.rates="survival",
                 sens=c(deltaR.c.survM,deltaIntra.c.survM,deltaInter.c.survM))

# merge
df4=rbind(df.h2,df.c2)

# save
write.csv(df4, "sens.per.vr.shurbs.MCMC.csv",row.names = F)
