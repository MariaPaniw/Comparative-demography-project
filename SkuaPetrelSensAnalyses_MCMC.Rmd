...
knitr::opts_knit$set(root.dir = "~/Files_Skua_and_Petrel")
```
...
# DENSITY
dens_s=read.table("Data_counts_skua.txt", quote="\"", comment.char="")
colnames(dens_s)="dens_s"
dens_p=read.table("Data_counts_petrel.txt", quote="\"", comment.char="")
...
for(u in 1:10){ # resampling parameters for uncertainties, therefore u
  for(i in 1:n.sim){ # 500 simulations to then get 1 mean lambda at the end
    for(t in 2:N) {
...
lambda_mean_s[i] <- mean(lambda_s) # lambda_s and lambda_p are there to store the lambdas for one run of the model for 10 years. So we run the 10-yr model 500 times. And store everytime the mean lambda (of each model, therefore mean of 10 years). Therefore, lambda_mean_s will have 500 lambdaas
lambda_mean_p[i] <- mean(lambda_p)
  
  }
  
lambda_mean_s=lambda_mean_s[101:500] # the first 100 years we discard
lambda_mean_p=lambda_mean_p[101:500]

lambda_mean_s=na.omit(lambda_mean_s) # omit NAs (there are only a few, like 3-6)
lambda_mean_p=na.omit(lambda_mean_p)

lambda_mean_s=mean(lambda_mean_s) # mean lambda of the 500 simulations
lambda_mean_p=mean(lambda_mean_p) # mean lambda of the 500 simulations

s_lambda[u]=lambda_mean_s # store lambdas from above but for 100 times, to get the uncertainties
p_lambda[u]=lambda_mean_p # same here as for the skua
}
...
# to store 100 deltas (to get uncertainties)
s.delta.SAM=NULL
p.delta.SAM=NULL
...
hist(s.delta.SAM-s.deltaSAM.cov)
```


### 4.3 Perturb SAM per vital rate

Here, I do the same as above but perturb covariates per vital rate. For example, I only perturb SAM for fecundity, and then for juvenile survival etc. (including covariation but that's just because we don't want things to be more complicated lol). Keep covariates in other vital rate functions at mean values.

#### 4.3.1 Survival for non-breeders
```{r}
# empty vector for lambdas
l_max_s=NULL
l_max_p=NULL
...
# empty vector for sensitivities
s.delta.SAM.NBsurv=NULL
p.delta.SAM.NBsurv=NULL
...
for(i in 1:n.sim){
  ...
  #### Apparent survival for non breeders at year t-1 ----
  ##### Skua ------
  logit_phiNB_s[t-1] <- mu.phiNB_s + delta_phiNB_s_SAM[u] * max_SAM + delta_phiNB_s_DD * (dens_s_when_SAM_max/100)  
  phiNB_s[t-1] <- 1/(1+exp(-logit_phiNB_s[t-1])) 

  NB_alive_s[t] <- rbinom(1, NB_s[t-1], phiNB_s[t-1]) 
  
  ##### Petrel ------
  logit_phiNB_p[t-1] <- mu.phiNB_p  + delta_phiNB_p_SAM[u] * max_SAM + delta_phiNB_p_SST[u] * SST_when_SAM_max + delta_phiNB_p_Chla[u] * Chla_when_SAM_max + delta_phiNB_p_DD * (dens_p_when_SAM_max/100) + delta_phiNB_p_PP * (dens_s_when_SAM_max/100)        # Survie Breeder
  phiNB_p[t-1] <- 1/(1+exp(-logit_phiNB_p[t-1]))
...
lambda_max_s=lambda_max_s[101:500]
lambda_max_p=lambda_max_p[101:500]
lambda_min_s=lambda_min_s[101:500]
lambda_min_p=lambda_min_p[101:500]

lambda_max_s=na.omit(lambda_max_s)
lambda_max_p=na.omit(lambda_max_p)
lambda_min_s=na.omit(lambda_min_s)
lambda_min_p=na.omit(lambda_min_p)

# the mean lambda of 400 simulations
lambda_max_s=mean(lambda_max_s) 
lambda_max_p=mean(lambda_max_p)
lambda_min_s=mean(lambda_min_s)
lambda_min_p=mean(lambda_min_p)


s.delta.SAM.NBsurv[u]=abs((lambda_max_s-lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.NBsurv[u]=abs((lambda_max_p-lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}

hist(s.delta.SAM.NBsurv)
hist(p.delta.SAM.NBsurv)

```

#### 4.3.2 Survival for breeders
```{r}
# empty vector for lambdas
l_max_s=NULL
l_max_p=NULL
...
# empty vector for sensitivities
s.delta.SAM.Bsurv=NULL
p.delta.SAM.Bsurv=NULL
for(u in 1:10){
# MAX
for(i in 1:n.sim){
  for(t in 2:N) {
  ...
  ##### Petrel ------
  logit_phiB_p[t-1] <- mu.phiB_p  + delta_phiB_p_Chla[u] * max_SAM + delta_phiB_p_SST[u] * SST_when_SAM_max + delta_phiB_p_Chla[u] * Chla_when_SAM_max + delta_phiB_p_DD * (dens_p_when_SAM_max/100) + delta_phiB_p_PP * (dens_s_when_SAM_max/100)        # Survie Breeder
  phiB_p[t-1] <- 1/(1+exp(-logit_phiB_p[t-1]))
...
hist(lambda_max_s, freq=F)
hist(lambda_max_p, freq=F)
...
s.delta.SAM.Bsurv[u]=abs((lambda_max_s-lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.Bsurv[u]=abs((lambda_max_p-lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}
```

#### 4.3.3 Reproduction of non-breeders
This includes 
for skua: breeding probability, successful breeding with one chick, with two chicks
for petrel: breeding probability, hatching success, successful breeding
```{r}
# empty vector for lambda
l_max_s=NULL
l_max_p=NULL
...
# empty vector for sensitivities
s.delta.SAM.NBreprod=NULL
p.delta.SAM.NBreprod=NULL
...
s.delta.SAM.NBreprod[u]=abs((lambda_max_s-lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.NBreprod[u]=abs((lambda_max_p-lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}

```

#### 4.3.4 Reproduction of breeders
This includes 
for skua: breeding probability, successful breeding with one chick, with two chicks
for petrel: breeding probability, hatching success, successful breeding
```{r}
...
# for sensitivities
s.delta.SAM.Breprod=NULL
p.delta.SAM.Breprod=NULL
...
s.delta.SAM.Breprod[u]=abs((lambda_max_s-lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.Breprod[u]=abs((lambda_max_p-lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
...
#### 4.3.5 Breeding probability of non-breeders
```{r}
# empty vector for lambda
l_max_s=NULL
l_max_p=NULL
...
s.delta.SAM.NBbreedingprobability=NULL
p.delta.SAM.NBbreedingprobability=NULL
...
s.delta.SAM.NBbreedingprobability[u]=abs((lambda_max_s-lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.NBbreedingprobability[u]=abs((lambda_max_p-lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}

```

#### 4.3.6 Breeding probability of breeders
```{r}
...
s.delta.SAM.Bbreedingprobability=NULL
p.delta.SAM.Bbreedingprobability=NULL
...
s.delta.SAM.Bbreedingprobability[u]=abs((lambda_max_s-lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.Bbreedingprobability[u]=abs((lambda_max_p-lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}
```

It almost doesn't matter whether I perturb all reproduction rates and just call it "reproduction" or I perturb only breeding probabilty (therefore decompose reproduction). The first one gives me a delta of p.delta.SAM.NBreprod = 0.001755986 vs p.delta.SAM.NBbreedingprobability = 0.001470464
So I don't really know if I should perturb allll the vital rates (the code is right now super long and complicated anyways) --> maybe do it at the end man idk
...
### 5.3 Perturb skua density per vital rate
 
#### 5.3.1 Survival for non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
s.delta.DensS.NBsurv=NULL
p.delta.DensS.NBsurv=NULL
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  s.delta.DensS.NBsurv[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_dens_s-min_dens_s)/sd_dens_s))
  p.delta.DensS.NBsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_s-min_dens_s)/sd_dens_s))
}
```

#### 5.3.2 Survival for breeders
```{r}
...
s.delta.DensS.Bsurv=NULL
p.delta.DensS.Bsurv=NULL
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  s.delta.DensS.Bsurv[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_dens_s-min_dens_s)/sd_dens_s))
  p.delta.DensS.Bsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_s-min_dens_s)/sd_dens_s))
}
```

#### 5.3.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
s.delta.DensS.NBreprod=NULL
p.delta.DensS.NBreprod=NULL
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  s.delta.DensS.NBreprod[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_dens_s-min_dens_s)/sd_dens_s))
  p.delta.DensS.NBreprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_s-min_dens_s)/sd_dens_s))
}
```

#### 5.3.4 Reproduction of breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
s.delta.DensS.Breprod=NULL
p.delta.DensS.Breprod=NULL
...
s.delta.DensS.Breprod[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_dens_s-min_dens_s)/sd_dens_s))
p.delta.DensS.Breprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_s-min_dens_s)/sd_dens_s))
}
```
...
### 6.3 Perturb petrel density per vital rate

#### 6.3.1 Survival for non-breeders (only petrel)
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.DensP.NBsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_p-min_dens_p)/sd_dens_p))
}
hist(p.delta.DensP.NBsurv)
```

#### 6.3.2 Survival for breeders (only petrel)
```{r}
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.DensP.Bsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_p-min_dens_p)/sd_dens_p))
}

```


#### 6.3.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
s.delta.DensP.NBreprod=NULL
p.delta.DensP.NBreprod=NULL
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  s.delta.DensP.NBreprod[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_dens_p-min_dens_p)/sd_dens_p))
  p.delta.DensP.NBreprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_p-min_dens_p)/sd_dens_p))
}
```


#### 6.3.4 Reproduction of breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
s.delta.DensP.Breprod=NULL
p.delta.DensP.Breprod=NULL
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  s.delta.DensP.Breprod[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_dens_p-min_dens_p)/sd_dens_p))
  p.delta.DensP.Breprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_p-min_dens_p)/sd_dens_p))
}
```
...
### 7.3 Perturb SST per vital rate
#### 7.3.1 Survival for non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.SST.NBsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SST-min_SST)/sd_SST))

}
```

#### 7.3.2 Survival for breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.SST.Bsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_dens_p-min_dens_p)/sd_dens_p))
}

```

#### 7.3.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.SST.NBreprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SST-min_SST)/sd_SST))

}
```

#### 7.3.4 Reproduction of breeders
```{r}
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.SST.Breprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SST-min_SST)/sd_SST))
}
```
...
### 8.3 Perturb Chla per vital rate
#### 8.3.1 Survival of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.Chla.NBsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_Chla-min_Chla)/sd_Chla))
}
```



#### 8.3.2 Survival for breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.Chla.Bsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_Chla-min_Chla)/sd_Chla))
}
```

#### 8.3.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.Chla.NBreprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_Chla-min_Chla)/sd_Chla))
}
```

#### 8.3.4 Reproduction of breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
for(i in 1:n.sim){
  ...
  logit_bB_p[t-1]  <- mu.bB_p  + delta_bB_p_SAM[u]  * SAM_when_Chla_max + delta_bB_p_SST[u] * SST_when_Chla_min + delta_bB_p_Chla[u] * max_Chla + delta_bB_p_DD * (dens_p_when_Chla_max/100)
  bB_p[t-1] <- 1/(1+exp(-logit_bB_p[t-1]))
  ...
  logit_wB_p[t-1]  <- mu.wB_p  + delta_wB_p_SAM[u] * SAM_when_Chla_max + delta_wB_p_SST[u] * SST_when_Chla_min + delta_wB_p_Chla[u] * max_Chla + delta_wB_p_DD * (dens_p_when_Chla_max/100) + delta_wB_p_PP * (dens_s_when_Chla_max/100)
  wB_p[t-1] <- 1/(1+exp(-logit_wB_p[t-1]))
  ...
  logit_gB_p[t-1]  <- mu.gB_p  + delta_gB_p_SAM[u] * SAM_when_Chla_max + delta_gB_p_SST[u]  * SST_when_Chla_min + delta_gB_p_Chla[u] * max_Chla + delta_gB_p_DD  * (dens_p_when_Chla_max/100) + delta_gB_p_PP * (dens_s_when_Chla_max/100)
  gB_p[t-1] <- 1/(1+exp(-logit_gB_p[t-1]))
...
lambda_min_p[i] <- mean(l_min_p)
  ...
  p.delta.Chla.Breprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_Chla-min_Chla)/sd_Chla))
...
## 9) Save output
```{r}
# scaled sensitivities (skua)
sens.skua=data.frame(s.delta.SAM=s.delta.SAM,s.delta.DensS=s.delta.DensS,s.delta.DensP=s.delta.DensP,s.deltaSAM.cov=s.deltaSAM.cov,s.deltaDensS.cov=s.deltaDensS.cov,s.delta.DensP.cov=s.delta.DensP.cov)

#sens.skua=data.frame(scaled.sens.no.cov=c(s.delta.SAM,s.delta.DensS,s.delta.DensP),
#                     scaled.sens.cov=c(s.deltaSAM.cov,s.deltaDensS.cov,s.delta.DensP.cov),
#                       driver=c("SAM","intraD","interD"),
#                       vr="all")

# scaled sensitivities (petrel)
sens.petrel=data.frame(p.delta.SAM=p.delta.SAM,p.delta.DensS=p.delta.DensS,p.delta.DensP=p.delta.DensP,p.delta.SST=p.delta.SST,p.delta.Chla=p.delta.Chla,p.deltaSAM.cov=p.deltaSAM.cov,p.deltaDensS.cov=p.deltaDensS.cov,p.delta.DensP.cov=p.delta.DensP.cov,p.delta.SST.cov=p.delta.SST.cov,p.delta.Chla.cov=p.delta.Chla.cov)

#sens.petrel=data.frame(scaled.sens.no.cov=c(p.delta.SAM,p.delta.DensS,p.delta.DensP,p.delta.SST,p.delta.Chla),
#                       scaled.sens.cov=c(p.deltaSAM.cov,p.deltaDensS.cov,p.delta.DensP.cov,p.delta.SST.cov,p.delta.Chla.cov),
#                       driver=c("SAM","interD","intraD","SST","Chla"),
#                       vr="all")

# scaled sensitivities per vital rate (skua)
sens.skua.vr=data.frame(s.delta.SAM.NBsurv=s.delta.SAM.NBsurv,s.delta.SAM.Bsurv=s.delta.SAM.Bsurv,s.delta.SAM.NBreprod=s.delta.SAM.NBreprod,s.delta.SAM.Breprod=s.delta.SAM.Breprod,s.delta.DensS.NBsurv=s.delta.DensS.NBsurv,s.delta.DensS.Bsurv=s.delta.DensS.Bsurv,s.delta.DensS.NBreprod=s.delta.DensS.NBreprod,s.delta.DensS.Breprod=s.delta.DensS.Breprod,s.delta.DensP.NBreprod=s.delta.DensP.NBreprod,s.delta.DensP.Breprod=s.delta.DensP.Breprod)

#sens.skua.vr=data.frame(scaled.sens=c(s.delta.SAM.NBsurv,s.delta.SAM.Bsurv,s.delta.SAM.NBreprod,s.delta.SAM.Breprod,s.delta.DensS.NBsurv,s.delta.DensS.Bsurv,s.delta.DensS.NBreprod,s.delta.DensS.Breprod,s.delta.DensP.NBsurv,s.delta.DensP.Bsurv,s.delta.DensP.NBreprod,s.delta.DensP.Breprod),
#                        driver=c(rep("SAM",4),rep("intraD",4),rep("interD",4)),
#                        vr=c("Non-breeder survival","Breeder survival","Non-breeder reproduction","Breeder reproduction"))

# scaled sensitivities per vital rate (petrel)
sens.petrel.vr=data.frame(p.delta.SAM.NBsurv=p.delta.SAM.NBsurv,p.delta.SAM.Bsurv=p.delta.SAM.Bsurv,p.delta.SAM.NBreprod=p.delta.SAM.NBreprod,p.delta.SAM.Breprod=p.delta.SAM.Breprod,p.delta.DensP.NBsurv=p.delta.DensP.NBsurv,p.delta.DensP.Bsurv=p.delta.DensP.Bsurv,p.delta.DensP.NBreprod=p.delta.DensP.NBreprod,p.delta.DensP.Breprod=p.delta.DensP.Breprod,p.delta.DensS.NBsurv=p.delta.DensS.NBsurv,p.delta.DensS.Bsurv=p.delta.DensS.Bsurv,p.delta.DensS.NBreprod=p.delta.DensS.NBreprod,p.delta.DensS.Breprod=p.delta.DensS.Breprod,p.delta.SST.NBsurv=p.delta.SST.NBsurv,p.delta.SST.Bsurv=p.delta.SST.Bsurv,p.delta.SST.NBreprod=p.delta.SST.NBreprod,p.delta.SST.Breprod=p.delta.SST.Breprod,p.delta.Chla.NBsurv=p.delta.Chla.NBsurv,p.delta.Chla.Bsurv=p.delta.Chla.Bsurv,p.delta.Chla.NBreprod=p.delta.Chla.NBreprod,p.delta.Chla.Breprod=p.delta.Chla.Breprod)

#sens.petrel.vr=data.frame(scaled.sens=c(p.delta.SAM.NBsurv,p.delta.SAM.Bsurv,p.delta.SAM.NBreprod,p.delta.SAM.Breprod,p.delta.DensP.NBsurv,p.delta.DensP.Bsurv,p.delta.DensP.NBreprod,p.delta.DensP.Breprod,p.delta.DensS.NBsurv,p.delta.DensS.Bsurv,p.delta.DensS.NBreprod,p.delta.DensS.Breprod,p.delta.SST.NBsurv,p.delta.SST.Bsurv,p.delta.SST.NBreprod,p.delta.SST.Breprod,p.delta.Chla.NBsurv,p.delta.Chla.Bsurv,p.delta.Chla.NBreprod,p.delta.Chla.Breprod),
#                          driver=c(rep("SAM",4),rep("intraD",4),rep("interD",4),rep("SST",4),rep("Chla",4)),
#                          vr=c("Non-breeder survival","Breeder survival","Non-breeder reproduction","Breeder reproduction"))

#write.csv(sens.skua,"SensSkua_MCMC.csv")
#write.csv(sens.petrel,"SensPetrel_MCMC.csv")
#write.csv(sens.skua.vr,"SensSkuaVitalRates_MCMC.csv")
#write.csv(sens.petrel.vr,"SensPetrelVitalRates_MCMC.csv")
```
