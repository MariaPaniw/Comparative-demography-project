...
knitr::opts_knit$set(root.dir = "/Users/esinickin/Desktop/Comparative-demography-project-main/Files_ArcticFox")
```
...
l_ratio.SI[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
# log ratios
#     1. abs(log(lambda(mpm.max)/lambda(mpm.min)))
l_ratio.SI.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
# log ratios
#     1. abs(log(lambda(mpm.max)/lambda(mpm.min)))
l_ratio.RC[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
# log ratios
#     1. abs(log(lambda(mpm.max)/lambda(mpm.min)))
l_ratio.RC.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
# log ratios
#     1. abs(log(lambda(mpm.max)/lambda(mpm.min)))
l_ratio.G[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
# log ratios
#     1. abs(log(lambda(mpm.max)/lambda(mpm.min)))
l_ratio.G.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
### 5.1) Vital rate-specific but with covariation
```{r}
source("pert.functions.fox.mcmc.R")

# per vital rate with cov
delta.SI.S0.cov=NULL
delta.SI.Sj.cov=NULL
delta.SI.Sa.cov=NULL
delta.SI.Psi.cov=NULL
delta.SI.Rho.cov=NULL
delta.RC.S0.cov=NULL
delta.RC.Sj.cov=NULL
delta.RC.Sa.cov=NULL
delta.RC.Psi.cov=NULL
delta.RC.Rho.cov=NULL
delta.G.Sj.cov=NULL
...
# PERTURBATIONS PER VITAL RATE WITH COVARIATION (S0, Sj, Sa, Psi, Rho) ---------------------

# SEA ICE EXTENT
# S0 (denning survival)
pert.S0.max1=pert.S0(reindeerCov = RC_when_SI_max, seaIceCov = , gooseCov = mean.G, pert.seaIceCov = max.SI, pert.reindeerCov = RC_when_SI_max, HuntingLevel=1, i = i, randomEff="ignore")
...
sens=data.frame(study.doi="10.1002/ecs2.3546",
                   ...
                   mat=0.8, # generation time [Myhrvold et al. 2015]
                ...
                lambda.sim=0, # was lambda calculated analytically (0) or using simulation (1)?
                    study.length=22, # study length in years
                l_ratio=c(l_ratio.SI,l_ratio.SI.cov,
                          l_ratio.RC,l_ratio.RC.cov,
                          l_ratio.G,l_ratio.G.cov)) 
