            ...
            for(nuc in 1){
              ...
              if(nrow(temp.conejos) != 0){
                ...
                if(p==1){
                  ...
                  FmL=data.fine_gcm$Fm[data.fine_gcm$yearl%in%min.temp[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  ...
                  FmL=data.fine_gcm$Fm[data.fine_gcm$yearl%in%max.temp[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
...
write.csv(dens.rabbit,"abundance_rabbit_cov.csv",row.names = F)
