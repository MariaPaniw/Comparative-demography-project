...
# Pick one GCM as all models have similar values for historical data 
            ...
            for(nuc in 1){
              ...
              if(nrow(temp.conejos) != 0){
                ...
                if(p==1){
                  ...
                  WL=data.fine_gcm$W[data.fine_gcm$yearl%in%min.temp[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  ...
                  WL=data.fine_gcm$W[data.fine_gcm$yearl%in%max.temp[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
...
write.csv(dens.rabbit,"abundance_rabbit_cov_vr_temp.csv",row.names = F)


########################### DRY MONTHS JUVENILE SURVIVAL
...
for(x in 1: length(scen)){
  ...
  sub.max=max.FM.year[max.FM.year$gcm%in%scen[x],]
 
  sub2=data.fine_gcm[data.fine_gcm$gcm%in%scen[x],]
  
  max.Fm[x]=as.numeric(as.character(sub.max$yearl[sub.max$Fm==max(sub.max$Fm)]))
  min.Fm[x]=as.numeric(as.character(sub.max$yearl[sub.max$Fm==min(sub.max$Fm)]))[1]
  mean.temp[[x]]= round(aggregate(ta~month,mean,data=sub2))
          ...
          for(m in months){
            for(nuc in 1){
              ...
              if(nrow(temp.conejos) != 0){
                ...
                # obtain variables needed for function of survival and repr success
                FmL.mu=mean.Fm[[which(scen==s)]]$Fm[m]
                TaL=mean.temp[[which(scen==s)]]$ta[m]
                if(p==1){
                  FmL=data.fine_gcm$Fm[data.fine_gcm$yearl%in%min.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"] 
                  DL=data.fine_gcm$mdl[data.fine_gcm$yearl%in%min.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  DIL=data.fine_gcm$DI[data.fine_gcm$yearl%in%min.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  WL=data.fine_gcm$W[data.fine_gcm$yearl%in%min.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  
                }else{
                  FmL=data.fine_gcm$Fm[data.fine_gcm$yearl%in%max.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"] 
                  DL=data.fine_gcm$mdl[data.fine_gcm$yearl%in%max.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  DIL=data.fine_gcm$DI[data.fine_gcm$yearl%in%max.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                  WL=data.fine_gcm$W[data.fine_gcm$yearl%in%max.Fm[which(scen==s)]&data.fine_gcm$month%in%m&data.fine_gcm$gcm%in%s&data.fine_gcm$rcp%in%"historical"&data.fine_gcm$nucleus%in%"Donana-Aljarafe"]
                ...
                #Adult survival
                temp.conejos$surv[temp.conejos$estado%in%"A"]=rbinom(nrow(temp.conejos[temp.conejos$estado%in%"A", ]),1,PSA(dens,FmL.mu))
                temp.conejos$surv[temp.conejos$estado%in%"J"]=rbinom(nrow(temp.conejos[temp.conejos$estado%in%"J", ]),1,PSJ(densV2,FmL))
...
write.csv(dens.rabbit,"abundance_rabbit_cov_vr_Jsurv.csv",row.names = F)


########################### DRY MONTHS ADULT SURVIVAL
          ...
          for(m in months){
            for(nuc in 1){
              ...
              if(nrow(temp.conejos) != 0){
                ...
                #Adult survival
                temp.conejos$surv[temp.conejos$estado%in%"A"]=rbinom(nrow(temp.conejos[temp.conejos$estado%in%"A", ]),1,PSA(densV2,FmL))
                temp.conejos$surv[temp.conejos$estado%in%"J"]=rbinom(nrow(temp.conejos[temp.conejos$estado%in%"J", ]),1,PSJ(dens,FmL.mu))
...
write.csv(dens.rabbit,"abundance_rabbit_cov_vr_Asurv.csv",row.names = F)
