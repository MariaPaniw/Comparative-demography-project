...
filter(no_resampling, species == "Pinus pinaster")
...
plot.clim <- 
  ...
  # for species with no label
  geom_point(
    data = filter(mean.climate_df, !species %in% species_labels), 
    aes(x = mat_jit, y = sens_mean, color = cov), 
    alpha = 0.5, 
    size = 2) +
  # for species with no label
...
filter(mean.climate_df, species == "Prunella collaris")
