...
mean.climate_df <- climate_df %>%
  ...
  summarise(
    sens_mean = mean(sens),
    sens_lower = quantile(sens, 0.025),
    sens_upper = quantile(sens, 0.975)
  )
...
mean.climate_df <- climate_df %>%
  ...
  summarise(
    sens_mean = mean(sens),
    sens_lower = quantile(sens, 0.05),
    sens_upper = quantile(sens, 0.95)
  )

# OR

# standard deviation
mean.climate_df <- select(mean.climate_df, -sens_lower,-sens_upper)
...
# OR
#  averaging the data over each SPECIFIC climate driver perturbed (so take out the resampling) and then calculate SD

# take out resampling
no_resampling <- climate_df %>%
  group_by(species, mat, cov, dens, group, driver) %>%
  slice(1)

# now take the average
mean.climate_df <- no_resampling %>%
  filter(mat > -1) %>%
  group_by(species, mat, cov, dens, group) %>%
  summarise(
    sens_mean = mean(sens),
...
filter(no_resampling, species == "Vulpes lagopus")

mean.climate_df$sens_lower
mean.climate_df$sens_upper
...
# species to label
species_labels=c('Prunella collaris','Lavandula stoechas','Vulpes lagopus','Marmota flaviventer','Protea repens','Pinus pinaster','Fagus sylvatica','Suricata suricatta','Cinclus cinclus','Branta leucopsis','Cistus libanotis','Halimium halimifolium')
...
filter(mean.climate_df, species == "Vulpes lagopus")


# save as tiff file for publication
ggsave(
  filename = "main_plot_Fig1_2.5_97.5.tiff",
  plot = plot.clim,
  device = "tiff",
  dpi = 600,
  width = 10,
