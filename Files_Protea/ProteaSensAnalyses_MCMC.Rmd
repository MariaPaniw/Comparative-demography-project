...
# set wd
knitr::opts_knit$set(root.dir = "~/Documents/Master Thesis/pert_analyses/Protea")
```
...
l_ratio.temp[i]=abs(log(ipm.max/ipm.min))
...
l_ratio.temp.cov[i]=abs(log(ipm.max.cov/ipm.min.cov))
...
l_ratio.precip[i]=abs(log(ipm.max/ipm.min))
...
l_ratio.precip.cov[i]=abs(log(ipm.max.cov/ipm.min.cov))
}
...
AllSens=data.frame(species="Protea repens",
                   ...
                   lambda.sim=0,
                   study.length=3,
                   l_ratio=c(l_ratio.temp,l_ratio.temp.cov,l_ratio.precip,l_ratio.precip.cov))
...
rm(df)
df=read.csv("/Users/esinickin/Documents/Master Thesis/pert_analyses/Protea/SensVR_Protea.csv")
names(df)[names(df) == "gen.time"] <- "mat"
df$mat=4
df$lambda.sim=0
df$study.length=3
write.csv(df,"/Users/esinickin/Documents/Master Thesis/pert_analyses/Protea/SensVR_Protea.csv",row.names = F)
