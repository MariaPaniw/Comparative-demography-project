...
L_ss_cr  = function(TMaS_t1, lintrad,index) {                                           
  
  beta <- coef(MFL_sscr)
  Vb   <- vcov(MFL_sscr)
  ilink <- family(MFL_sscr)$linkinv
  
  if(index==1){temp=TMaS_t1}else{temp=mean.TMaS_t1}
  Xp = predict(MFL_sscr,                                               
               newdata=data.frame(TMaS_t1=temp, lintrad=lintrad),type="lpmatrix")
...
L_ss2_cr = function(Size, TMaS_t1, Site,index){                                         
  
  beta <- coef(MFL_ss2cr)
  Vb   <- vcov(MFL_ss2cr)
  ilink <- family(MFL_ss2cr)$linkinv
  
  if(index==2){temp=TMaS_t1}else{temp=mean.TMaS_t1}
  Xp = predict(MFL_ss2cr,                                               
               newdata=data.frame(TMaS_t1=temp, Site=Site),type="lpmatrix")
...
K.fnc_L_cr = function(midp, TMe, TMaS_t1,P_t1,lintrad, Site, Year,index) {
  ...
  Ss = matrix(as.vector(L_ss_cr(TMaS_t1=TMaS_t1, lintrad = lintrad,index=index)) *   # Size in which discrete stage is incorporated into continuous stage
                as.vector(L_ss2_cr(Size=midp$meshp, TMaS_t1=TMaS_t1,Site=Site,index=index)), 
...
for (i in 1:100){
  ...
  print(paste("Running iteration ",i))
      ...
      sens.out=rbind(sens.out,data.frame(spec.driver="TMaS_t1",
                                         driver="Temperature",
                                         driver.type="C",
                                         stage.age="juvenile",vital.rates="survival",
                                         sens=deltaTMaS_t1cov,
                                         l_ratio=abs(log(lambda(mpm.max)/lambda(mpm.min))),
                                         cov=1,
                                         sim=i))
      
      # TMaS_t1 cov juvenile size
      # min
      mpm.min = K.fnc_L_cr (
        midp = midp, 
        TMe = mean.TMe,
      ...
      sens.out=rbind(sens.out,data.frame(spec.driver="TMaS_t1",
                                         driver="Temperature",
                                         driver.type="C",
                                         stage.age="juvenile",vital.rates="size",
                                         sens=deltaTMaS_t1cov,
                                         l_ratio=abs(log(lambda(mpm.max)/lambda(mpm.min))),
                                         cov=1,
                                         sim=i))
      ...
      sens.out=rbind(sens.out,data.frame(spec.driver="P_t1",
                                         driver="Rain",
                                         driver.type="C",
                                         stage.age="all",vital.rates="reproduction",
                                         sens=deltaP_t1cov,
                                         l_ratio=abs(log(lambda(mpm.max)/lambda(mpm.min))),
                                         cov=1,
                                         sim=i))
...
sens_lav=data.frame(species="Lavandula stoechas", study.doi="Sanchez Mejia in prep",year.of.publication=NA,
                           ...
                           stage.age=sens.out$stage.age,vital.rates=sens.out$vital.rates,sens=sens.out$sens,mat=3,n.vr=7,n.pam=35,dens=0,
                           biotic_interactions=0,lambda.sim=0,study.length=5)

write.csv(sens_lav,"sens_lavandula_vr.csv",row.names = F)
