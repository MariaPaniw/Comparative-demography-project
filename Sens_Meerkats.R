...
# define empty lists for sensitivities
delta.R=NULL
delta.R.V2=NULL
delta.T=NULL
delta.T.V2=NULL
...
track_progress <- function(iteration, total) {
  percent <- round((iteration / total) * 100)
  cat(sprintf('\r[%-20s] %3d%%', 
              paste0(rep('=', percent %/% 5), 
                     rep(' ', 20 - percent %/% 5)), 
              percent))
}
...
# STEP 3: SENS ###############
### 1. Sensitivity to rain no covariation ---------------
lambda.R=NULL
...
for(j in 1:2){
  ...
  for(i in 1:12){
    ...
    tempSD=mean.temp
    rainSD=mu.cov$rainSD[mu.cov$year==rain[j]][i]
    
    #### REGULAR IPM
    
    density=mu.cov.month$density[i]
  ...
  lambda.R[j]=get.eigen.stuff(as.matrix(IPM.full))
...
### 2. Sensitivity to rain full covariation ----------------
lambda.R.V2=NULL
...
for(j in 1:2){
  ...
  for(i in 1:12){
    ...
    IPM=array(0,c(n.bins*n.stage,n.bins*n.stage))
    ### Get observed values for given year and month
    tempSD=mu.cov$tempSD[mu.cov$year==rain[j]][i]
    rainSD=mu.cov$rainSD[mu.cov$year==rain[j]][i]
    
    #### REGULAR IPM
    
    density=mu.cov$density[mu.cov$year==rain[j]][i]
  ...
  lambda.R.V2[j]=get.eigen.stuff(as.matrix(IPM.full))
...
for(j in 1:2){
  ...
  for(i in 1:12){
    ...
    IPM=array(0,c(n.bins*n.stage,n.bins*n.stage))
    ### Get observed values for given year and month
    tempSD=mu.cov$tempSD[mu.cov$year==temp[j]][i]
    rainSD=mean.rain
  ...
  lambda.T[j]=get.eigen.stuff(as.matrix(IPM.full))
...
for(j in 1:2){
  ...
  for(i in 1:12){
    ...
    ### Get observed values for given year and month
    tempSD=mu.cov$tempSD[mu.cov$year==temp[j]][i]
    rainSD=mu.cov$rainSD[mu.cov$year==temp[j]][i]
    
    #### REGULAR IPM
    
    density=mu.cov$density[mu.cov$year==temp[j]][i]
  ...
  lambda.T.V2[j]=get.eigen.stuff(as.matrix(IPM.full))
...
for(j in 1:2){
  ...
  lambda.D[j]=get.eigen.stuff(as.matrix(IPM.full))
...
for(j in 1:2){
  ...
  for(i in 1:12){
    ...
    IPM=array(0,c(n.bins*n.stage,n.bins*n.stage))
    ### Get observed values for given year and month
    tempSD=mu.cov$tempSD[mu.cov$year==dens[j]][i]
    rainSD=mu.cov$rainSD[mu.cov$year==dens[j]][i]
  ...
  lambda.D.V2[j]=get.eigen.stuff(as.matrix(IPM.full))
...
# STEP 4: SAVE OUTPUT #########################
sens.meerkats=data.frame(delta.R=delta.R,
                         delta.R.V2=delta.R.V2,
                         delta.T=delta.T,
                         delta.T.V2=delta.T.V2,
                         delta.D=delta.D,
                         delta.D.V2=delta.D.V2)

write.csv(sens.meerkats, "fifty_iterations_sens2.csv", row.names = F)
