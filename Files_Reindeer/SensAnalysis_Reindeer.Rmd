...
# set wd
knitr::opts_knit$set(root.dir = "~/Desktop/Comparative-demography-project-main/Files_Reindeer")
```
...
for(i in 1:100){
  ...
  Sens.ROS.NoCov[i] <- abs((lambda(mpm.max)-lambda(mpm.min))/((max.ROS-min.ROS)/sd.ROS))
  l_ratio.ROS.NoCov[i] <- abs(log(lambda(mpm.max)/lambda(mpm.min)))
}
...
for(i in 1:100){
  ...
  Sens.ROS.Cov[i] <- abs((lambda(mpm.max)-lambda(mpm.min))/((max.ROS-min.ROS)/sd.ROS))
   l_ratio.ROS.Cov[i] <- abs(log(lambda(mpm.max)/lambda(mpm.min)))
}
...
for(i in 1:100){
  ...
  Sens.Dens.NoCov[i] <- abs((lambda(mpm.max)-lambda(mpm.min))/((max.dens-min.dens)/sd.dens))
   l_ratio.Dens.NoCov[i] <- abs(log(lambda(mpm.max)/lambda(mpm.min)))
}
...
for(i in 1:100){
  ...
  Sens.Dens.Cov[i] <- abs((lambda(mpm.max)-lambda(mpm.min))/((max.dens-min.dens)/sd.dens))
   l_ratio.Dens.Cov[i] <- abs(log(lambda(mpm.max)/lambda(mpm.min)))
}
...
ScaledSens_Reindeer=data.frame(species="Rangifer tarandus",
                      ...
                      group="Mammals",
                      continent="Europe",
                      driver=rep(c("ROS","Dens"),each=200),
                      driver.type=rep(c("C","D"),each=200),
                      stage.age="all",
                      vital.rates="all",
                      sens=c(Sens.ROS.NoCov,Sens.ROS.Cov,Sens.Dens.NoCov,Sens.Dens.Cov),
                      ...
                      lambda.sim=0,
                      study.length=20,
                     l_ratio=c(l_ratio.ROS.NoCov,l_ratio.ROS.Cov,l_ratio.Dens.NoCov,l_ratio.Dens.Cov))
