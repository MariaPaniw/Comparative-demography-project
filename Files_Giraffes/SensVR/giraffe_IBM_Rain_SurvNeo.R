    ...
    for(i in 1:(seasons-1)){ # second loop: SEASONS
      ...
      for(gg in 1:length(groups)){ # third loop: GROUP, which is social community
        ...
        if(nrow(DATA[DATA$age%in%c(1:3),])>0){
          ...
          DATA$surv[DATA$age%in%c(1:3)]=rbinom(length(DATA$surv[DATA$age%in%c(1:3)]),prob=survNeo(DATA$age[DATA$age%in%c(1:3)],DATA$age[DATA$age%in%c(1:3)]^2,cov.data$DerAbund[cov.data$rain==rain&cov.data$Community==gg],DATA$beta3[DATA$age%in%c(1:3)],rain,com.effjuv.vec[gg], 
        ...
        if(nrow(DATA[DATA$age%in%c(4:12),])>0){
          ...
          DATA$surv[DATA$age%in%c(4:12)]=rbinom(length(DATA$surv[DATA$age%in%c(4:12)]),prob=survJuv(DATA$age[DATA$age%in%c(4:12)],DATA$age[DATA$age%in%c(4:12)]^2,ab.comm$DerAbund[gg],DATA$beta3[DATA$age%in%c(4:12)],mean.rain,com.effjuv.vec[gg]),size=1)
        }
...
# The output of this simulation is saved as "abund.pert.rain.cov.csv" 

write.csv(AB.DATA,"abund.rain.survneo.csv",row.names = F)
# save as abund.rain.survneo.csv