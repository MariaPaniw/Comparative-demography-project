...
setwd("~/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Master Thesis/pert_analyses/Giraffes")
    ...
    for(i in 1:(seasons-1)){ # second loop: SEASONS
      ...
      for(gg in 1:length(groups)){ # third loop: GROUP, which is social community
        ...
        if(nrow(DATA[DATA$age%in%c(13:87),])>0){
          ...
          DATA$surv[DATA$age%in%c(13:87)]=rbinom(length(DATA$surv[DATA$age%in%c(13:87)]),prob=survAd(curr.seas.char[i],rain,cov.data$DerAbund[cov.data$rain==rain&cov.data$Community==gg],com.effaf.vec[gg]),size=1)
        }
...
# save output as abund.rain.survad.csv
write.csv(AB.DATA,"abund.rain.survad.csv",row.names = F)
