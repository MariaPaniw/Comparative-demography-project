...
cov.data=read.csv("/datos/giraffe_data/abund_validation2023.csv")  # load observed rainfall anomalies
    ...
    for(i in 1:(seasons-1)){ # second loop: SEASONS
      ...
      for(gg in 1:length(groups)){ # third loop: GROUP, which is social community
        ...
        if(nrow(DATA[DATA$age%in%c(1:3),])>0){
          ...
          DATA$surv[DATA$age%in%c(1:3)]=rbinom(length(DATA$surv[DATA$age%in%c(1:3)]),prob=survNeo(DATA$age[DATA$age%in%c(1:3)],DATA$age[DATA$age%in%c(1:3)]^2,ab.comm$DerAbund[gg],DATA$beta3[DATA$age%in%c(1:3)],rain,com.effjuv.vec[gg], 
        ...
        if(nrow(DATA[DATA$age%in%c(4:12),])>0){
          ...
          DATA$surv[DATA$age%in%c(4:12)]=rbinom(length(DATA$surv[DATA$age%in%c(4:12)]),prob=survJuv(DATA$age[DATA$age%in%c(4:12)],DATA$age[DATA$age%in%c(4:12)]^2,ab.comm$DerAbund[gg],DATA$beta3[DATA$age%in%c(4:12)],rain,com.effjuv.vec[gg]),size=1)
        }
        ...
        if(nrow(DATA[DATA$age%in%c(13:87),])>0){
          ...
          DATA$surv[DATA$age%in%c(13:87)]=rbinom(length(DATA$surv[DATA$age%in%c(13:87)]),prob=survAd(curr.seas.char[i],rain,ab.comm$DerAbund[gg],com.effaf.vec[gg]),size=1)
        }
...
# The output of this simulation is saved as "abund.pert.rain.csv" 