summer_pertGnra_ipm <- function(year,env,env.pert,year.pert,i){
  ...
  # SURVIVAL 
  S2y <- diag(S2.fun(z,stage=1,year,Q.sim),nrow=n,ncol=n,i) # Survival yearlings
  S2nra <- diag(S2.fun(z,stage=2,year,Q.sim),nrow=n,ncol=n,i) # Survival non-reproductive adults 
  S2ra <- diag(S2.fun(z,stage=3,year,Q.sim),nrow=n,ncol=n,i) # Survival reproductive adults 
  ...
  # Reproductive Adults 
  Gnra <- h*t(outer(z,z,GR2.fun,stage=2,year.sim.pert,Q.sim.pert,i)) 
