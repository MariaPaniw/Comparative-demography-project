...
````{r setup, include = F}
knitr::opts_knit$set(root.dir="/Users/esinickin/Desktop/Comparative-demography-project-main/Files_Marmot")
...
for(i in 1:100){
  ...
  # log ratios: abs(log(lambda(mpm.max)/lambda(mpm.min)))
  l_ratio.Q[i]=abs(log(lambda(max.ipm)/lambda(min.ipm)))
  ...
  l_ratio.Q.cov[i]=abs(log(lambda(max.ipm)/lambda(min.ipm)))
}
...
sens_df=data.frame(study.doi="10.1111/ele.13459",
                 ...
                 study.length=40,
                 l_ratio=c(l_ratio.Q,l_ratio.Q.cov)
                 )

write.csv(sens_df,"/Users/esinickin/Desktop/Comparative-demography-project-main/Files_Marmot/SENS_MARMOTS_MCMC.csv",row.names = F)
