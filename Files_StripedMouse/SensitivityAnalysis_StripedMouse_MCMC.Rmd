...
knitr::opts_knit$set(root.dir = "~/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Master Thesis/pert_analyses/StripedMouse")
```
...
all.sens <- data.frame(study.doi="10.1111/1365-2656.12888",
                   ...
                   continent="Africa",
                   driver=rep(c("temperature","food availability","density","lagged temperature","lagged food availability","lagged density"),each=100),
                   driver.type=rep(c("C","B","D","C","B","D"),each=100),
                   stage.age="all",
                   vital.rates="all",
                   sens.no.cov=c(delta.T,delta.F,delta.D,delta.T_t,delta.F_t,delta.D_t),
                   sens.cov=c(deltaT.cov,deltaF.cov,deltaD.cov,deltaT_t.cov,deltaF_t.cov,deltaD_t.cov))
...
write.csv(sens.vr,"Sens_per_VR_StripedMouse_MCMC.csv",row.names = F)
```
...
cov=expand.grid(temp=seq(min.temp,max.temp,length.out=5),
                food=seq(min.food,max.food,length.out=5),
                dens_F=seq(min.dens_F,max.dens_F,length.out=5),
                temp_t=seq(min.temp_t,max.temp_t,length.out=5),
                food_t=seq(min.food_t,max.food_t,length.out=5),
                dens_F_t=seq(min.dens_F_t,max.dens_F_t,length.out=5))

# empty object for lambdas
all.l=rep(NA,nrow(cov))

for(s in 1:nrow(cov)){
  mpm=my.matrix(cov$temp[s],cov$temp_t[s],cov$food[s],cov$food_t[s],cov$dens_F[s],cov$dens_F_t[s])
...
source("pert.functions.striped.mouse.R")
```
...
for(s in 1:length(stable.l)){
  ...
  temp_t=cov$temp_t[stable.l[s]]
  temp_t.pert=cov$temp_t[stable.l[s]]+0.1*abs(cov$temp_t[stable.l[s]]) # increase temp_t by 10%
  food=cov$food[stable.l[s]]
  food_t=cov$food_t[stable.l[s]]
  dens_F=cov$dens_F[stable.l[s]]
  dens_F_t=cov$dens_F_t[stable.l[s]]
  
  control=my.matrix(temp,temp_t,food,food_t,dens_F,dens_F_t)
  pert.Si=pert.Si.temp(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert)
  pert.Sp=pert.Sp.temp(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert)
  pert.Sb=pert.Sb.temp(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert)
  pert.PsiIB=pert.PsiIB.temp(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert)
  pert.PsiPB=pert.PsiPB.temp(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert)
  pert.Bp=pert.Bp.tempt(temp,temp_t,food,food_t,dens_F,dens_F_t,temp_t.pert)
  pert.Lp=pert.Lp.tempt(temp,temp_t,food,food_t,dens_F,dens_F_t,temp_t.pert)
  pert.LS=pert.LS.tempt(temp,temp_t,food,food_t,dens_F,dens_F_t,temp_t.pert)
  pert.all=pert.all.temp(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert,temp_t.pert)
  
  temp.mpm=data.frame(vr=c("Si","Sp","Sb","PsiIB","PsiPB","Bp","Lp","LS","All"),
                      delta = c((lambda(pert.Si)-lambda(control))/lambda(control),
                                (lambda(pert.Sp)-lambda(control))/lambda(control),
                                (lambda(pert.Sb)-lambda(control))/lambda(control),
                                (lambda(pert.PsiIB)-lambda(control))/lambda(control),
                                (lambda(pert.PsiPB)-lambda(control))/lambda(control),
                                (lambda(pert.Bp)-lambda(control))/lambda(control),
                                (lambda(pert.Lp)-lambda(control))/lambda(control),
                                (lambda(pert.LS)-lambda(control))/lambda(control),
                                (lambda(pert.all)-lambda(control))/lambda(control)))
  dT.mouse=rbind(dT.mouse,temp.mpm)
}
dT.mouse$variable="Temp"
dT.mouse$vr=factor(dT.mouse$vr, levels=c("Si","Sp","Sb","PsiIB","PsiPB","Bp","Lp","LS","All"))
...
for(s in 1:length(stable.l)){
  ...
  temp_t=cov$temp_t[stable.l[s]]
  food=cov$food[stable.l[s]]
  food.pert=cov$food[stable.l[s]]+0.1*abs(cov$food[stable.l[s]]) # increase temp by 10%
  food_t=cov$food_t[stable.l[s]]
  food_t.pert=cov$food_t[stable.l[s]]+0.1*abs(cov$food_t[stable.l[s]]) # increase temp by 10%
  dens_F=cov$dens_F[stable.l[s]]
  dens_F_t=cov$dens_F_t[stable.l[s]]
  
  control=my.matrix(temp,temp_t,food,food_t,dens_F,dens_F_t)
  pert.Si=pert.Si.food(temp,temp_t,food,food_t,dens_F,dens_F_t,food.pert)
  pert.Sp=pert.Sp.food(temp,temp_t,food,food_t,dens_F,dens_F_t,food.pert)
  pert.Sb=pert.Sb.food(temp,temp_t,food,food_t,dens_F,dens_F_t,food.pert)
  pert.PsiIB=pert.PsiIB.food(temp,temp_t,food,food_t,dens_F,dens_F_t,food.pert)
  pert.PsiPB=pert.PsiPB.food(temp,temp_t,food,food_t,dens_F,dens_F_t,food.pert)
  pert.Bp=pert.Bp.foodt(temp,temp_t,food,food_t,dens_F,dens_F_t,food_t.pert)
  pert.Lp=pert.Lp.foodt(temp,temp_t,food,food_t,dens_F,dens_F_t,food_t.pert)
  pert.LS=pert.LS.foodt(temp,temp_t,food,food_t,dens_F,dens_F_t,food_t.pert)
  pert.all=pert.all.food(temp,temp_t,food,food_t,dens_F,dens_F_t,food.pert,food_t.pert)
  
  food.mpm=data.frame(vr=c("Si","Sp","Sb","PsiIB","PsiPB","Bp","Lp","LS","All"),
                      delta = c((lambda(pert.Si)-lambda(control))/lambda(control),
                                (lambda(pert.Sp)-lambda(control))/lambda(control),
                                (lambda(pert.Sb)-lambda(control))/lambda(control),
                                (lambda(pert.PsiIB)-lambda(control))/lambda(control),
                                ...
                                (lambda(pert.LS)-lambda(control))/lambda(control),
                                (lambda(pert.all)-lambda(control))/lambda(control)))
  dF.mouse=rbind(dF.mouse,food.mpm)
}
dF.mouse$variable="Food"
...
for(s in 1:length(stable.l)){
  ...
  food_t=cov$food_t[stable.l[s]]
  dens_F=cov$dens_F[stable.l[s]]
  dens_F.pert=cov$dens_F[stable.l[s]]+0.1*abs(cov$dens_F[stable.l[s]]) # increase temp by 10%
  dens_F_t=cov$dens_F_t[stable.l[s]]
  dens_F_t.pert=cov$dens_F_t[stable.l[s]]+0.1*abs(cov$dens_F_t[stable.l[s]]) # increase temp by 10%
  
  control=my.matrix(temp,temp_t,food,food_t,dens_F,dens_F_t)
  pert.Si=pert.Si.densF(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F.pert)
  pert.Sp=pert.Sp.densF(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F.pert)
  pert.Sb=pert.Sb.densF(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F.pert)
  pert.PsiIB=pert.PsiIB.densF(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F.pert)
  pert.PsiPB=pert.PsiPB.densF(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F.pert)
  pert.Bp=pert.Bp.densFt(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F_t.pert)
  pert.Lp=pert.Lp.densFt(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F_t.pert)
  pert.LS=pert.LS.densFt(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F_t.pert)
  pert.all=pert.all.densF(temp,temp_t,food,food_t,dens_F,dens_F_t,dens_F.pert,dens_F_t.pert)
  
  densF.mpm=data.frame(vr=c("Si","Sp","Sb","PsiIB","PsiPB","Bp","Lp","LS","All"),
                      delta = c((lambda(pert.Si)-lambda(control))/lambda(control),
                                (lambda(pert.Sp)-lambda(control))/lambda(control),
                                (lambda(pert.Sb)-lambda(control))/lambda(control),
                                (lambda(pert.PsiIB)-lambda(control))/lambda(control),
                                ...
                                (lambda(pert.LS)-lambda(control))/lambda(control),
                                (lambda(pert.all)-lambda(control))/lambda(control)))
  dD.mouse=rbind(dD.mouse,densF.mpm)
}
dD.mouse$variable="DensF"
...
for(s in 1:length(stable.l)){
  ...
  temp.pert=cov$temp[stable.l[s]]+0.1*abs(cov$temp[stable.l[s]]) # increase temp by 10%
  temp_t=cov$temp_t[stable.l[s]]
  temp_t.pert=cov$temp_t[stable.l[s]]+0.1*abs(cov$temp_t[stable.l[s]]) # increase temp by 10%
  food=cov$food[stable.l[s]]
  food.pert=cov$food[stable.l[s]]+0.1*abs(cov$food[stable.l[s]]) # increase temp by 10%
  food_t=cov$food_t[stable.l[s]]
  food_t.pert=cov$food_t[stable.l[s]]+0.1*abs(cov$food_t[stable.l[s]]) # increase temp by 10%
  ...
  control=my.matrix(temp,temp_t,food,food_t,dens_F,dens_F_t)
  pert.Si=pert.Si.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert,food.pert,dens_F.pert)
  pert.Sp=pert.Sp.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert,food.pert,dens_F.pert)
  pert.Sb=pert.Sb.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert,food.pert,dens_F.pert)
  pert.PsiIB=pert.PsiIB.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert,food.pert,dens_F.pert)
  pert.PsiPB=pert.PsiPB.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp.pert,food.pert,dens_F.pert)
  pert.Bp=pert.Bp.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp_t.pert,food_t.pert,dens_F_t.pert)
  pert.Lp=pert.Lp.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp_t.pert,food_t.pert,dens_F_t.pert)
  pert.LS=pert.LS.all(temp,temp_t,food,food_t,dens_F,dens_F_t,temp_t.pert,food_t.pert,dens_F_t.pert)
  pert.all=pert.all.all(temp.pert,temp_t.pert,food.pert,food_t.pert,dens_F.pert,dens_F_t.pert)
  
  all.mpm=data.frame(vr=c("Si","Sp","Sb","PsiIB","PsiPB","Bp","Lp","LS","All"),
                     delta = c((lambda(pert.Si)-lambda(control))/lambda(control),
                               (lambda(pert.Sp)-lambda(control))/lambda(control),
                               (lambda(pert.Sb)-lambda(control))/lambda(control),
                               (lambda(pert.PsiIB)-lambda(control))/lambda(control),
                               ...
                               (lambda(pert.LS)-lambda(control))/lambda(control),
                               (lambda(pert.all)-lambda(control))/lambda(control)))
  dA.mouse=rbind(dA.mouse,all.mpm)
}
dA.mouse$variable="All"

# put all results into a dataframe
df=rbind(dT.mouse,dF.mouse,dD.mouse,dA.mouse)
df$vr=factor(df$vr,levels=c("Si","Sp","Sb","PsiIB","PsiPB","Bp","Lp","LS","All"))
...
Here it's a bit weird... not sure

# COMMENT MARIA: Do you mean they are *more* pronounced? Your code looks correct. So, it maybe just be that if we perturb eveything simultaneously, the results become quite varied because there are just so many things we are perturbing.  
```{r}
library(viridis)
library(ggplot2)
...
Here are the sensitivities in numbers: 
```{r}
library(dplyr)
# temperature
print("Temp")
mean(filter(df, vr == "Si" & variable == "Temp")$delta)
mean(filter(df, vr == "Sp" & variable == "Temp")$delta)
mean(filter(df, vr == "Sb" & variable == "Temp")$delta)
mean(filter(df, vr == "PsiIB" & variable == "Temp")$delta)
mean(filter(df, vr == "PsiPB" & variable == "Temp")$delta)
# Food
print("Food")
mean(filter(df, vr == "Si" & variable == "Food")$delta)
mean(filter(df, vr == "Sp" & variable == "Food")$delta)
mean(filter(df, vr == "Sb" & variable == "Food")$delta)
mean(filter(df, vr == "PsiIB" & variable == "Food")$delta)
mean(filter(df, vr == "PsiPB" & variable == "Food")$delta)
# DensF
print("DensF")
mean(filter(df, vr == "Si" & variable == "DensF")$delta)
mean(filter(df, vr == "Sp" & variable == "DensF")$delta)
mean(filter(df, vr == "Sb" & variable == "DensF")$delta)
mean(filter(df, vr == "PsiIB" & variable == "DensF")$delta)
mean(filter(df, vr == "PsiPB" & variable == "DensF")$delta)
# All
print("All")
mean(filter(df, vr == "Si" & variable == "All")$delta)
mean(filter(df, vr == "Sp" & variable == "All")$delta)
mean(filter(df, vr == "Sb" & variable == "All")$delta)
mean(filter(df, vr == "PsiIB" & variable == "All")$delta)
mean(filter(df, vr == "PsiPB" & variable == "All")$delta)
```
