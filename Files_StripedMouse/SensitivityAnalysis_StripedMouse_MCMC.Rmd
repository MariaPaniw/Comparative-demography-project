...
knitr::opts_knit$set(root.dir = "~/Desktop/Comparative-demography-project-main/Files_StripedMouse")
```
...
l_ratio.T.cov=NULL
l_ratio.T_t=NULL
l_ratio.T_t.cov=NULL

l_ratio.F=NULL
l_ratio.F.cov=NULL
l_ratio.F_t=NULL
l_ratio.F_t.cov=NULL

l_ratio.D=NULL
l_ratio.D.cov=NULL
l_ratio.D_t=NULL
l_ratio.D_t.cov=NULL
...
for(i in 1:100){
  ...
  delta.T_t[i]=abs((lambda(mpm.max)-lambda(mpm.min))/((max.temp_t-min.temp_t)/sd.temp_t))
   l_ratio.T_t[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
  ...
  deltaT_t.cov[i]=abs((lambda(mpm.max)-lambda(mpm.min))/((max.temp_t-min.temp_t)/sd.temp_t))
   l_ratio.T_t.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
  ...
  delta.F[i]=abs((lambda(mpm.max)-lambda(mpm.min))/((max.food-min.food)/sd.food))
   l_ratio.F[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
 ...
 l_ratio.F.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
 ...
 l_ratio.F_t[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
  ...
  deltaF_t.cov[i]=abs((lambda(mpm.max)-lambda(mpm.min))/((max.food_t-min.food_t)/sd.food_t))
   l_ratio.F_t.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
  ...
  delta.D_t[i]=abs((lambda(mpm.max)-lambda(mpm.min))/((max.dens_F_t-min.dens_F_t)/sd.dens_F_t))
   l_ratio.D_t[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
  ...
  deltaD_t.cov[i]=abs((lambda(mpm.max)-lambda(mpm.min))/((max.dens_F_t-min.dens_F_t)/sd.dens_F_t))
   l_ratio.D_t.cov[i]=abs(log(lambda(mpm.max)/lambda(mpm.min)))
...
all.sens <- data.frame(study.doi="10.1111/1365-2656.12888",
                   ...
                   cov=rep(c(0,1),100),
                    mat=0.7, # age at maturity [Schradin et al. 2012]
                   ...
                   l_ratio=c(l_ratio.T,l_ratio.T.cov,
                          l_ratio.F,l_ratio.F.cov,
                          l_ratio.D,l_ratio.D.cov,
                          l_ratio.T_t,l_ratio.T_t.cov,
                          l_ratio.F_t,l_ratio.F_t.cov,
                          l_ratio.D_t,l_ratio.D_t.cov))
...
sens.vr$lambda.sim=0
sens.vr$study.length=9
