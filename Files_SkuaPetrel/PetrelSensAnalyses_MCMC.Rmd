...
knitr::opts_knit$set(root.dir = "~/Documents/Master Thesis/pert_analyses/SkuaPetrel")
...
load("/Users/esinickin/Documents/Master Thesis/pert_analyses/SkuaPetrel/mcmc_env_cov_reg_coef.Rdata")
...
# Time series
cov=data.frame(year=1996:2017,
               SAM_b_s,SAM_d_s,SAM_g_s,SAM_phi_s, # SAM skua
               SAM_b_p,SAM_g_p,SAM_phi_p,SAM_w_p, # SAM petrel
               SST_phi_p,SST_b_p,SST_w_p,SST_g_p, # SST petrel 
               Chla_phi_p,Chla_b_p,Chla_w_p,Chla_g_p, # Chla petrel
               dens_s,dens_p) # dens skua and petrel

# plot SAM for petrel
plot(cov$SAM_b_p, type="l")
lines(cov$SAM_g_p, col="brown")
lines(cov$SAM_phi_p, col="blue")
lines(cov$SAM_w_p, col="orange")
title("SAM for petrel")
legend("topright", 
       legend=c("SAM_b_p", "SAM_g_p", "SAM_phi_p", "SAM_w_p"),
       col=c("black", "brown", "blue", "orange"))

# plot SST for petrel
plot(cov$SST_b_p, type = "l")
lines(cov$SST_g_p, col = "brown")
lines(cov$SAM_phi_p, col = "blue")
lines(cov$SST_w_p, col = "orange")
title("SST for petrel")
legend("topright", 
       legend=c("SST_b_p", "SST_g_p", "SST_phi_p", "SST_w_p"),
       col=c("black", "brown", "blue", "orange"))

# plot Chla for petrel
plot(cov$Chla_b_p,type="l")
```
...
for(u in 1:10){ # sample parameter from distribution only 10 times
  ...
  longterm_s[u] <- lambda_s[11:20]
  longterm_p[u] <- lambda_p[11:20]
...
# Sensitivities to...
to different environmental, abiotic, or biotic variables. All vital rates are perturbed (Numbers 4 - 8).
...
for(u in 1:50){ # 50 resampling for uncertainties
  ...
  p.l_ratio.SAM[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}  
...
# log ratio for petrel
p.l_ratio.SAM.cov=NULL
...
for(u in 1:50){ # 50 resamples for uncertainties 
  ...
  p.l_ratio.SAM.cov[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
...
for(u in 1:50){
  ...
  p.l_ratio.DensS[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
s.delta.DensS
p.delta.DensS
```
...
for(u in 1:50){
  ...
  p.l_ratio.DensS.cov[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
```
...
for(u in 1:50){
  ...
  p.l_ratio.DensP[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
```
...
for(u in 1:50){
  ...
  p.l_ratio.DensP.cov[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
  }
```
...
for(u in 1:50){
  ...
  p.l_ratio.SST[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
```
...
for(u in 1:50){
  ...
  p.l_ratio.SST.cov[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
```
...
for(u in 1:50){
  ...
  p.l_ratio.Chla[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
...
for(u in 1:50){
  ...
  p.l_ratio.Chla.cov[u]=abs(log(mean_lambda_max_p/mean_lambda_min_p))
}
```
...
s.delta.SAM=na.omit(s.delta.SAM)[1:10]
...
p.delta.SAM=na.omit(p.delta.SAM)[1:10]
...
# log ratios for petrel
p.l_ratio.SAM=na.omit(p.l_ratio.SAM)[1:10]
p.l_ratio.SAM.cov=na.omit(p.l_ratio.SAM.cov)[1:10]

p.l_ratio.DensS=na.omit(p.l_ratio.DensS)[1:10]
p.l_ratio.DensS.cov=na.omit(p.l_ratio.DensS.cov)[1:10]

p.l_ratio.DensP=na.omit(p.l_ratio.DensP)[1:10]
p.l_ratio.DensP.cov=na.omit(p.l_ratio.DensP.cov)[1:10]

p.l_ratio.SST=na.omit(p.l_ratio.SST)[1:10]
p.l_ratio.SST.cov=na.omit(p.l_ratio.SST.cov)[1:10]

p.l_ratio.Chla=na.omit(p.l_ratio.Chla)[1:10]
p.l_ratio.Chla.cov=na.omit(p.l_ratio.Chla.cov)[1:10]
...
#                   driver=c(rep(c("SAM","IntraDens","InterDens"),each=20)),
#                   driver.type=c(rep(c("C","D","B"),each=20)),
#                   stage.age="all",
#                   vital.rates="all",
#                   sens=c(s.delta.SAM,s.deltaSAM.cov,s.delta.DensS,
#                          s.deltaDensS.cov,s.delta.DensP,s.deltaDensP.cov),
...
# 
# write.csv(s.df, "SensSkua_MCMC.csv", row.names = F)
...
p.df <-  data.frame(study.doi="10.1002/ecm.1459",
                  ...
                  continent="Sub-Antarctic",
                  driver = c(rep(c("SAM","IntraDens","InterDens","SST","Chla"),each=20)),
                  driver.type=c(rep(c("C","D","B","C","B"),each=20)),
                  stage.age="all",
                  vital.rates="all",
                  sens=c(p.delta.SAM,p.deltaSAM.cov,p.delta.DensP,p.delta.DensP.cov,p.delta.DensS,p.deltaDensS.cov,p.delta.SST,p.delta.SST.cov,p.delta.Chla,p.delta.Chla.cov),
                   cov=rep(c(0,1),each=10),
                  mat=4,
                  n.vr=8, # number of vital rates with covariates
                  n.pam=46, # number of total parameters of these vital rates
                  dens=1,
                  biotic_interactions=1,
                  lambda.sim=1,
                  study.length=22,
                  l_ratio=c(p.l_ratio.SAM,p.l_ratio.SAM.cov,
                            p.l_ratio.DensP,p.l_ratio.DensP.cov,
                            p.l_ratio.DensS,p.l_ratio.DensS.cov,
                            p.l_ratio.SST,p.l_ratio.SST.cov,
                            p.l_ratio.Chla,p.l_ratio.Chla.cov))

write.csv(p.df, "SensPetrel_MCMC.csv", row.names = F)

```


# Sensitivties per vital rate

Here we compute the scaled sensitivities to the same variables similar above, but this time only perturbing one vital rate at a time.

## 10) SAM
#### 10.1 Survival for non-breeders
```{r}
N <- 20
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
...
# take mean of 20 years for resampling (50)
mean_lambda_max_s=NULL
mean_lambda_max_p=NULL
mean_lambda_min_s=NULL
mean_lambda_min_p=NULL
...
for(u in 1:50){ # 50 resampling for uncertainties
  ...
  s.delta.SAM.NBsurv[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
  p.delta.SAM.NBsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}

```

#### 10.2 Survival for breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
# MIN
  ...
  # store the mean of the lambdas 10-19
  mean_lambda_max_s=mean(l_max_s[10:19])
  mean_lambda_max_p=mean(l_max_p[10:19])
  ...
  # to store the 50 sensitivities because of resampling
  s.delta.SAM.Bsurv[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
  p.delta.SAM.Bsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}

```


#### 10.3 Reproduction of non-breeders
This includes 
...
s.delta.SAM.NBreprod[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.NBreprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
}

```

#### 10.4 Reproduction of breeders
This includes 
for skua: breeding probability, successful breeding with one chick, with two chicks
for petrel: breeding probability, hatching success, successful breeding
```{r}
...
s.delta.SAM.Breprod[u]=abs((mean_lambda_max_s-mean_lambda_min_s)/((max_SAM-min_SAM)/sd_SAM))
p.delta.SAM.Breprod[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SAM-min_SAM)/sd_SAM))
...
## 11) Skua density
#### 11.1 Survival for non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
#### 11.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
## 12) Petrel density
#### 12.1 Survival for non-breeders (only petrel)
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
#### 12.2 Survival for breeders (only petrel)
```{r}
...
#### 12.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
## 13) SST (only petrel)
#### 13.1 Survival for non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
for(u in 1:50){
 ...
 # store the mean of the lambdas 11-20
  ...
  p.delta.SST.Bsurv[u]=abs((mean_lambda_max_p-mean_lambda_min_p)/((max_SST-min_SST)/sd_SST))
}

```


#### 13.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
#### 14.3 Reproduction of non-breeders
```{r}
# empty vector for lambdas
# to calculate lambdas for 10 years: l_max_s[t-1] <- Nadtot_s[t]/Nadtot_s[t-1]
l_max_s=NULL 
...
## 15) SAVE OUTPUT
```{r}
# first omit the NA's and only get 10 resamples because that's enough

# skua
s.delta.SAM.NBsurv=na.omit(s.delta.SAM.NBsurv)[1:10]
s.delta.SAM.Bsurv=na.omit(s.delta.SAM.Bsurv)[1:10]
s.delta.SAM.NBreprod=na.omit(s.delta.SAM.NBreprod)[1:10]
s.delta.SAM.Breprod=na.omit(s.delta.SAM.Breprod)[1:10]
s.delta.DensS.NBsurv=na.omit(s.delta.DensS.NBsurv)[1:10]
s.delta.DensS.Bsurv=na.omit(s.delta.DensS.Bsurv)[1:10]
s.delta.DensS.NBreprod=na.omit(s.delta.DensS.NBreprod)[1:10]
s.delta.DensS.Breprod=na.omit(s.delta.DensS.Breprod)[1:10]
s.delta.DensP.NBreprod=na.omit(s.delta.DensP.NBreprod)[1:10]
s.delta.DensP.Breprod=na.omit(s.delta.DensP.Breprod)[1:10]

# petrel
p.delta.SAM.NBsurv=na.omit(p.delta.SAM.NBsurv)[1:10]
p.delta.SAM.Bsurv=na.omit(p.delta.SAM.Bsurv)[1:10]
p.delta.SAM.NBreprod=na.omit(p.delta.SAM.NBreprod)[1:10]
p.delta.SAM.Breprod=na.omit(p.delta.SAM.Breprod)[1:10]
p.delta.DensS.NBsurv=na.omit(p.delta.DensS.NBsurv)[1:10]
p.delta.DensS.Bsurv=na.omit(p.delta.DensS.Bsurv)[1:10]
p.delta.DensS.NBreprod=na.omit(p.delta.DensS.NBreprod)[1:10]
p.delta.DensS.Breprod=na.omit(p.delta.DensS.Breprod)[1:10]
p.delta.DensP.NBreprod=na.omit(p.delta.DensP.NBreprod)[1:10]
p.delta.DensP.Breprod=na.omit(p.delta.DensP.Breprod)[1:10]
p.delta.DensP.NBsurv=na.omit(p.delta.DensP.NBsurv)[1:10]
p.delta.DensP.Bsurv=na.omit(p.delta.DensP.Bsurv)[1:10]
p.delta.SST.NBreprod=na.omit(p.delta.SST.NBreprod)[1:10]
p.delta.SST.Breprod=na.omit(p.delta.SST.Breprod)[1:10]
p.delta.SST.NBsurv=na.omit(p.delta.SST.NBsurv)[1:10]
p.delta.SST.Bsurv=na.omit(p.delta.SST.Bsurv)[1:10]
p.delta.Chla.NBreprod=na.omit(p.delta.Chla.NBreprod)[1:10]
p.delta.Chla.Breprod=na.omit(p.delta.Chla.Breprod)[1:10]
p.delta.Chla.NBsurv=na.omit(p.delta.Chla.NBsurv)[1:10]
p.delta.Chla.Bsurv=na.omit(p.delta.Chla.Bsurv)[1:10]


# # skua
# s.df.vr <- data.frame(study.doi="10.1002/ecm.1459",
#                   year.of.publication="2021",
#                   group="Birds",
#                   species="Catharacta loennbergi",
#                   continent="Sub-Antarctic",
#                   driver=rep(c("SAM","SAM","SAM","SAM",
#                                  "IntraDens","IntraDens","IntraDens","IntraDens",
#                                  "InterDens","InterDens"),each=10),
#                   driver.type=rep(c("C","C","C","C",
#                                  "D","D","D","D",
#                                  "B","B"),each=10),
#                   stage.age=rep(c("previous non-breeder","previous breeder"),each=10),
#                   vital.rates=rep(c("survival","reproduction",
#                                     "survival","reproduction",
#                                     "survival","reproduction",
#                                     "survival","reproduction",
#                                     "reproduction","reproduction"),each=10),
#                   sens=c(s.delta.SAM.NBsurv,s.delta.SAM.Bsurv,
#                          s.delta.SAM.NBreprod,s.delta.SAM.Breprod,
#                          
#                          s.delta.DensS.NBsurv,s.delta.DensS.Bsurv,
#                          s.delta.DensS.NBreprod,s.delta.DensS.Breprod,
#                          
#                          s.delta.DensP.NBreprod,s.delta.DensP.Breprod),
#                   gen.time=4,
#                   n.vr=8, # number of vital rates with covariates
#                   n.pam=28, # number of total parameters of these vital rates
#                   dens=1,
#                   biotic_interactions=1)
# 
# write.csv(s.df.vr,"SensSkua_VR_MCMC.csv", row.names = F)


# petrel
p.df.vr <-  data.frame(study.doi="10.1002/ecm.1459",
                  year.of.publication="2021",
                  group="Birds",
                  species="Halobaena caerulea",
                  continent="Sub-Antarctic",
                  driver=rep(c("SAM","InterDens","IntraDens","SST","Chla"),each=40),
                  driver.type=rep(c("C","B","D","C","B"),each=40),
                  stage.age=rep(c("previous non-breeder","previous breeder"),each=10),
                  vital.rates=rep(c("survival","reproduction"),each=20),
                  sens=c(p.delta.SAM.NBsurv,p.delta.SAM.Bsurv,
                         p.delta.SAM.NBreprod,p.delta.SAM.Breprod,
                         
                         p.delta.DensS.NBsurv,p.delta.DensS.Bsurv,
                         p.delta.DensS.NBreprod,p.delta.DensS.Breprod,
                         
                         p.delta.DensP.NBsurv,p.delta.DensP.Bsurv,
                         p.delta.DensP.NBreprod,p.delta.DensP.Breprod,
                         
                         p.delta.SST.NBsurv,p.delta.SST.Bsurv,
                         p.delta.SST.NBreprod,p.delta.SST.Breprod,
                         
                         p.delta.Chla.NBsurv,p.delta.Chla.Bsurv,
                         p.delta.Chla.NBreprod,p.delta.Chla.Breprod),
                  mat=4,
                  n.vr=8, # number of vital rates with covariates
                  n.pam=46, # number of total parameters of these vital rates
                  dens=1,
...
write.csv(p.df.vr,"SensPetrel_VR_MCMC.csv", row.names = F)
