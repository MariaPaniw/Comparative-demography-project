...
# The covariates are set to: max temperature and everything else to their historical values when temperature was at its maximum
...
if(!load.workspace){
  ...
  for(i in 1:nrow(aux.data)){
    if(aux.data$predicted.basal.area[i]>0 & aux.data$saplings[i]>0){
      ...
      # ESIN
      q <- c(cov$Precip_when_Temp_max[my.plot],cov$max_temp[my.plot],plot.predicted.ba$ba[my.plot],cov$AnomPrecip_when_Temp_max[my.plot],cov$AnomTemp_when_Temp_max[my.plot])
...
for(iyear in seq(2000,final.year,10)){
  ...
  for (i in 1:NUM_PLOTS) {
    ...
    if (sum(ba[i,])>0 | sum(sapl)>0){
      ...
      temper <- cov$max_temp[my.id[i]]
      rain <- cov$Precip_when_Temp_max[my.id[i]]
      anom.temper <- cov$AnomTemp_when_Temp_max[my.id[i]]  
      anom.rain <- cov$AnomPrecip_when_Temp_max[my.id[i]]
...
if(ALL.RESULTS){
  ...
  temperature <- cov$max_temp[match(results[[1]]$ID,cov$ID)]
  precipitation <- cov$Precip_when_Temp_max[match(results[[1]]$ID,cov$ID)]
...
# put the results of each simulation run into a separate folder in the results folder "MaxTempCov"
# example: run 5
for (i in 1:16) {
  write.csv(results[[i]], paste0("MaxTempCov/run5/results.sp", i, ".csv"),row.names = F)
}
