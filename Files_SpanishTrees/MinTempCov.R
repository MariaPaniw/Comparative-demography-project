...
# The covariates are set to: minimum temperature and everything else to their historical values when temperature was at its minimum
# to calculate sensitivities to temperature assuming covariation among covariates
...
if(!load.workspace){
  ...
  for(i in 1:nrow(aux.data)){
    if(aux.data$predicted.basal.area[i]>0 & aux.data$saplings[i]>0){
      ...
      # ESIN
      q <- c(cov$Precip_when_Temp_min[my.plot],cov$min_temp[my.plot],plot.predicted.ba$ba[my.plot],cov$AnomPrecip_when_Temp_min[my.plot],cov$AnomTemp_when_Temp_min[my.plot])
...
for(iyear in seq(2000,final.year,10)){
  ...
  for (i in 1:NUM_PLOTS) {
    ...
    if (sum(ba[i,])>0 | sum(sapl)>0){
      ...
      temper <- cov$min_temp[my.id[i]]
      rain <- cov$Precip_when_Temp_min[my.id[i]]
      anom.temper <- cov$AnomTemp_when_Temp_min[my.id[i]]  
      anom.rain <- cov$AnomPrecip_when_Temp_min[my.id[i]]
...
if(ALL.RESULTS){
  ...
  temperature <- cov$min_temp[match(results[[1]]$ID,cov$ID)]
  precipitation <- cov$Precip_when_Temp_min[match(results[[1]]$ID,cov$ID)]
...
# put the results of each simulation run into a separate folder in the results folder "MinTempCov"
# example: run 5

for (i in 1:16) {
  write.csv(results[[i]], paste0("MinTempCov/run5/results.sp", i, ".csv"),row.names = F)
}
