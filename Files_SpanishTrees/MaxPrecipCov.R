...
# The covariates are set to: max precipitation and everything else to their historical values when precipitation was at its maximum
# to calculate sensitivities to precipitation assuming covariation among covariates
...
if(!load.workspace){
  ...
  for(i in 1:nrow(aux.data)){
    if(aux.data$predicted.basal.area[i]>0 & aux.data$saplings[i]>0){
      ...
      # ESIN
      q <- c(cov$max_precip[my.plot],cov$Temp_when_Precip_max[my.plot],plot.predicted.ba$ba[my.plot],cov$AnomPrecip_when_Precip_max[my.plot],cov$AnomTemp_when_Precip_max[my.plot])
...
for(iyear in seq(2000,final.year,10)){
  ...
  for (i in 1:NUM_PLOTS) {
    ...
    if (sum(ba[i,])>0 | sum(sapl)>0){
      ...
      temper <- cov$Temp_when_Precip_max[my.id[i]]
      rain <- cov$max_precip[my.id[i]]
      anom.temper <- cov$AnomTemp_when_Precip_max[my.id[i]]  
      anom.rain <- cov$AnomPrecip_when_Precip_max[my.id[i]]
...
if(ALL.RESULTS){
  ...
  temperature <- cov$Temp_when_Precip_max[match(results[[1]]$ID,cov$ID)]
  precipitation <- cov$max_precip[match(results[[1]]$ID,cov$ID)]
...
# put the results of each simulation run into a separate folder in the results folder "MaxPrecipCov"
# example: run 5

for (i in 1:16) {
  write.csv(results[[i]], paste0("MaxPrecipCov/run5/results.sp", i, ".csv"),row.names = F)
}
