...
# remove species 4 it doesn't work

# Prepare session #####################
# set wd
setwd("/Users/esinickin/Library/CloudStorage/OneDrive-UniversitätZürichUZH/Master Thesis/pert_analyses/SpainTrees")
...
df2$full.species <- c("Conifers","Deciduous","Fagus sylvatica","Pinus halepensis","Pinus nigra","Pinus pinaster","Pinus pinea","Pinus sylvestris","Pinus uncinata",
                            "Quercus faginea","Quercus ilex","Quercus pyrenaica",
...
# filter out the groups "conifers", "deciduous" and "sclerophyllous"
df2 <- dplyr::filter(df2, !(species %in% c("Conifers", "Deciduous", "Sclerophyllous")))

# add species-specific generation time
df.gentime=data.frame(species=c("Fagus sylvatica","Pinus halepensis","Pinus nigra","Pinus pinaster","Pinus pinea","Pinus sylvestris","Pinus uncinata",
                                "Quercus faginea","Quercus ilex","Quercus pyrenaica","Quercus robur/petraea","Quercus suber"),
                      gen.time=c(30,7,15,6,15,5,10,30,30,30,30,12)) # see supporting information for sources
df2=merge(df2, df.gentime, by="species",all.x=T)

# add other important variables regarding the IPM  
df2$n.vr=4  # number of vital rates
df2$n.pam=6 # number of parameters
df2$vr_par=24 # product of the above two
