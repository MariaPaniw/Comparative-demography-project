...
# The covariates are set to: minimum precipitation and everything else to their historical values when precipitation was at its minimum
...
if(!load.workspace){
  ...
  for(i in 1:nrow(aux.data)){
    if(aux.data$predicted.basal.area[i]>0 & aux.data$saplings[i]>0){
      ...
      # ESIN
      q <- c(cov$min_precip[my.plot],cov$Temp_when_Precip_min[my.plot],plot.predicted.ba$ba[my.plot],cov$AnomPrecip_when_Precip_min[my.plot],cov$AnomTemp_when_Precip_min[my.plot])
...
for(iyear in seq(2000,final.year,10)){
  ...
  for (i in 1:NUM_PLOTS) {
    ...
    if (sum(ba[i,])>0 | sum(sapl)>0){
      ...
      temper <- cov$Temp_when_Precip_min[my.id[i]]
      rain <- cov$min_precip[my.id[i]]
      anom.temper <- cov$AnomTemp_when_Precip_min[my.id[i]]  
      anom.rain <- cov$AnomPrecip_when_Precip_min[my.id[i]]
...
if(ALL.RESULTS){
  ...
  temperature <- cov$Temp_when_Precip_min[match(results[[1]]$ID,cov$ID)]
...
# put the results of each simulation run into a separate folder in the results folder "MinPrecipCov"
# example: run 5
for (i in 1:16) {
  write.csv(results[[i]], paste0("MinPrecipCov/run5/results.sp", i, ".csv"),row.names = F)
}
