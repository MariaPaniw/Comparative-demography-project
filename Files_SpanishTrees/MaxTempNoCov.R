...
# The covariates are set to: maximum temperature and everything else to mean values across the study period
# to calculate sensitivities to temperature assuming mean covariates aka no covariation
...
if(!load.workspace){
  ...
  for(i in 1:nrow(aux.data)){
    if(aux.data$predicted.basal.area[i]>0 & aux.data$saplings[i]>0){
      ...
      q <- c(cov$max_precip[my.plot],cov$mean_temp[my.plot],plot.predicted.ba$ba[my.plot],cov$mean_anom_pl[my.plot],cov$mean_anom_temp[my.plot])
...
if(ALL.RESULTS){
  ...
  precipitation <- cov$mean_precip[match(results[[1]]$ID,cov$ID)]
...
sub=results[[1]]
sub[sub$adults_2000>0,]
# ESIN:
# save output as files results.spi.csv where i = ID of species
# put the results of each simulation run into a separate folder in the results folder "MaxTempNoCov"
# example: run 5

for (i in 1:16) {
  write.csv(results[[i]], paste0("MaxTempNoCov/run5/results.sp", i, ".csv"),row.names = F)
}
