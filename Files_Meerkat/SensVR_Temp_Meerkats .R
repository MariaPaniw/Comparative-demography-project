...
Pups.Gr.T=NULL # 1. Sensitivity of growth of pups to temp
Pups.Surv.T=NULL # 2. Sensitivity of survival of pups to temp

Juv.Gr.T=NULL # 3. Sensitivity of growth of juveniles to temp
Juv.Surv.T=NULL # 4. Sensitivity of survival of juveniles to temp

Sub.Gr.T=NULL # 5. Sensitivity of growth of subadults to temp
Sub.Surv.T=NULL # 6. Sensitivity of survival of subadults to temp

H.G.T=NULL # 7. Sensitivity of growth of helpers to temp
H.Surv.T=NULL # 8. Sensitivity of survival of helpers to temp

D.G.T=NULL # 9. Sensitivity of growth of dominants to temp
D.Surv.T=NULL # 10. Sensitivity of survival of dominants to temp

H.Rec.T=NULL # 11. Sensitivity of helper recruitment rate to temp
D.Rec.T=NULL # 12. Sensitivity of dominant recruitment rate to temp
...
for(pi in 1:iterations){
  ...
  ### 1. Sens of pup growth to temp ---------------
  lambdaT.1=NULL
  
  temp=as.character(c(min.temp,max.temp))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      tempSD=mean.temp
      rainSD=mean.rain
      pert.tempSD=mu.cov$tempSD[mu.cov$year==temp[j]][i] # max or min temp
      ...
      G=h*t(outer(z,z,f.pup.gr,ageM.pup,i,rainSD,pert.tempSD,density,temp[j]))
      ...
      G=h*t(outer(z,z,f.juv.gr,ageM.juv,i,rainSD,tempSD,density,temp[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      ...
      G=h*t(outer(z,z,f.sub.gr,ageM.sub,i,rainSD,tempSD,density,temp[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      ...
      S=diag(f.help.surv(z,i,density,rainSD,tempSD,temp[j])$pred)
      Shelp1=S
      E=diag(f.help.emig(z,i,density,rainSD,tempSD,temp[j])$pred)
      
      Ph_d=diag(f.toDom(z,i,density,rainSD)$pred)
      
      ### not pregnant -> staying not pregnant helper
      pregCat="np"
      G=h*t(outer(z,z,f.help.gr,pregCat,i,rainSD,tempSD,density,temp[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      Ghelp1=G
      Thh.np.p1=diag(f.help.NPH(z,i,density,rainSD,tempSD,temp[j])$pred)
      ...
      G[is.na(G)]=0
      Ghelp2=G
      Thh.p1.p2=diag(f.help.FH(z,i,density,rainSD,tempSD,temp[j])$pred)
      ...
      G[is.na(G)]=0
      Ghelp3=G
      Thh.p2.b=diag(f.help.SH(z,i,density,temp[j])$pred)
      ...
      Thh.p2.p1=diag(f.help.SH2(z,i,density,rainSD,tempSD,temp[j])$pred)
      ...
      pregCat="np"
      G=h*t(outer(z,z,f.dom.gr,pregCat,i,rainSD,tempSD,density,temp[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      
      Td.np.p1=diag(f.dom.NP(z,i,density,rainSD,tempSD,temp[j])$pred)
      ...
      if(temp[j]=="1997"|temp[j]=="1998"){
        
        Td.p1.p1=diag(f.dom.F2(z,i,density,rainSD,tempSD,"1999")$pred)
      }else{ Td.p1.p1=diag(f.dom.F2(z,i,density,rainSD,tempSD,temp[j])$pred) }
      ...
      Td.p2.b=diag(f.dom.Sec(z,i,density,rainSD,tempSD,temp[j])$pred)
      ...
      Td.b.np=diag(f.dom.B(z,i,density,rainSD,tempSD,temp[j])$pred)
      
      IPM[(16*n.bins+1):(17*n.bins),(19*n.bins+1):(20*n.bins)]=G%*%Td.b.np
      
      ### after birth -> pregnant 1 month
      Td.b.p1=diag(f.dom.B2(z,i,density,rainSD,tempSD,temp[j])$pred)
      ...
      Rhelp=R
      D=h*t(outer(z,z,f.help.off.mass,i,rainSD,tempSD,density,temp[j]))
      ...
      D=h*t(outer(z,z,f.dom.off.mass,i,rainSD,tempSD,density,temp[j]))
      D=D/matrix(as.vector(apply(D,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      D[is.na(D)]=0
    ...
    lambdaT.1[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  Pups.Gr.T[pi]=abs((lambdaT.1[2]-lambdaT.1[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### Age 1 Pups
      ageM.pup=ageM.pup.tot[1]
      G=h*t(outer(z,z,f.pup.gr,ageM.pup,i,rainSD,tempSD,density,temp[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.pup.surv(z,ageM.pup,i,rainSD,pert.tempSD,density)$pred)
      IPM[(n.bins+1):(2*n.bins),1:n.bins]=G%*%S
    ...
    lambdaT.2[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  Pups.Surv.T[pi]=abs((lambdaT.2[2]-lambdaT.2[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=h*t(outer(z,z,f.juv.gr,ageM.juv,i,rainSD,pert.tempSD,density,temp[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.juv.surv(z,density,rainSD,tempSD)$pred)
    ...
    lambdaT.3[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  Juv.Gr.T[pi]=abs((lambdaT.3[2]-lambdaT.3[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.juv.surv(z,density,rainSD,pert.tempSD)$pred)
    ...
    lambdaT.4[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  Juv.Surv.T[pi]=abs((lambdaT.4[2]-lambdaT.4[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=h*t(outer(z,z,f.sub.gr,ageM.sub,i,rainSD,pert.tempSD,density,temp[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.sub.surv(z,ageM.sub,i,density,rainSD,tempSD)$pred)
    ...
    lambdaT.5[j]=get.eigen.stuff(as.matrix(IPM.full))
  ...
  Sub.Gr.T[pi]=abs((lambdaT.5[2]-lambdaT.5[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.sub.surv(z,ageM.sub,i,density,rainSD,pert.tempSD)$pred)
    ...
    lambdaT.6[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  Sub.Surv.T[pi]=abs((lambdaT.6[2]-lambdaT.6[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### not pregnant -> staying not pregnant helper
      pregCat="np"
      G=h*t(outer(z,z,f.help.gr,pregCat,i,rainSD,pert.tempSD,density,temp[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      Ghelp1=G
      Thh.np.p1=diag(f.help.NPH(z,i,density,rainSD,tempSD,temp[j])$pred)
    ...
    lambdaT.7[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  H.G.T[pi]=abs((lambdaT.7[2]-lambdaT.7[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      S=diag(f.help.surv(z,i,density,rainSD,pert.tempSD,temp[j])$pred)
      Shelp1=S
      E=diag(f.help.emig(z,i,density,rainSD,tempSD,temp[j])$pred)
    ...
    lambdaT.8[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  H.Surv.T[pi]=abs((lambdaT.8[2]-lambdaT.8[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### non-pregnant -> non-pregnant
      pregCat="np"
      G=h*t(outer(z,z,f.dom.gr,pregCat,i,rainSD,pert.tempSD,density,temp[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
    ...
    lambdaT.9[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  D.G.T[pi]=abs((lambdaT.9[2]-lambdaT.9[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### DOMINANT
      S=diag(f.dom.surv(z,i,density,rainSD,pert.tempSD)$pred)
    ...
    lambdaT.10[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  D.Surv.T[pi]=abs((lambdaT.10[2]-lambdaT.10[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      R=diag(as.numeric(f.help.pups(z,i,density,rainSD)$pred),n.bins)
      Rhelp=R
      D=h*t(outer(z,z,f.help.off.mass,i,rainSD,pert.tempSD,density,temp[j]))
      D=D/matrix(as.vector(apply(D,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      D[is.na(D)]=0
      Dhelp=D
      IPM[1:n.bins,(15*n.bins+1):(16*n.bins)]=D%*%R
    ...
    lambdaT.11[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  H.Rec.T[pi]=abs((lambdaT.11[2]-lambdaT.11[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      tempSD=mean.temp
      rainSD=mean.rain
      pert.tempSD=mu.cov$tempSD[mu.cov$year==temp[j]][i] # max or min rain
      ...
      # from dominant
      R=diag(f.dom.pups(z,density)$pred)
      D=h*t(outer(z,z,f.dom.off.mass,i,rainSD,pert.tempSD,density,temp[j]))
      D=D/matrix(as.vector(apply(D,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      D[is.na(D)]=0
    ...
    lambdaT.12[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  D.Rec.T[pi]=abs((lambdaT.12[2]-lambdaT.12[1])/((mean(mu.cov$tempSD[mu.cov$year==temp[2]])-mean(mu.cov$tempSD[mu.cov$year==temp[1]]))/1))
...
write.csv(Results_all,"SensVR_Temp_Meerkats.csv",row.names = F)
