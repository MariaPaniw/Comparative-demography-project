...
Pups.Gr.D=NULL # 1. Sensitivity of growth of pups to temp
Pups.Surv.D=NULL # 2. Sensitivity of survival of pups to temp

Juv.Gr.D=NULL # 3. Sensitivity of growth of juveniles to temp
Juv.Surv.D=NULL # 4. Sensitivity of survival of juveniles to temp

Sub.Gr.D=NULL # 5. Sensitivity of growth of subadults to temp
Sub.Surv.D=NULL # 6. Sensitivity of survival of subadults to temp

H.G.D=NULL # 7. Sensitivity of growth of helpers to temp
H.Surv.D=NULL # 8. Sensitivity of survival of helpers to temp

D.G.D=NULL # 9. Sensitivity of growth of dominants to temp
D.Surv.D=NULL # 10. Sensitivity of survival of dominants to temp

H.Rec.D=NULL # 11. Sensitivity of helper recruitment rate to temp
D.Rec.D=NULL # 12. Sensitivity of dominant recruitment rate to temp
...
for(pi in 1:iterations){
  ...
  ### 1. Sens of pup growth to density ---------------
  lambdaD.1=NULL
  
  dens=as.character(c(min.density,max.density))
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      pert.density=mu.cov$density[mu.cov$year==dens[j]][i]
      
      density=mu.cov.month$density[i] # density of the month
      
      ### Age 1 Pups
      ageM.pup=ageM.pup.tot[1]
      G=h*t(outer(z,z,f.pup.gr,ageM.pup,i,rainSD,tempSD,pert.density,dens[j]))
      ...
      G=h*t(outer(z,z,f.juv.gr,ageM.juv,i,rainSD,tempSD,density,dens[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      ...
      G=h*t(outer(z,z,f.sub.gr,ageM.sub,i,rainSD,tempSD,density,dens[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      ...
      S=diag(f.help.surv(z,i,density,rainSD,tempSD,dens[j])$pred)
      Shelp1=S
      E=diag(f.help.emig(z,i,density,rainSD,tempSD,dens[j])$pred)
      
      Ph_d=diag(f.toDom(z,i,density,rainSD)$pred)
      
      ### not pregnant -> staying not pregnant helper
      pregCat="np"
      G=h*t(outer(z,z,f.help.gr,pregCat,i,rainSD,tempSD,density,dens[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      Ghelp1=G
      Thh.np.p1=diag(f.help.NPH(z,i,density,rainSD,tempSD,dens[j])$pred)
      ...
      G[is.na(G)]=0
      Ghelp2=G
      Thh.p1.p2=diag(f.help.FH(z,i,density,rainSD,tempSD,dens[j])$pred)
      ...
      G[is.na(G)]=0
      Ghelp3=G
      Thh.p2.b=diag(f.help.SH(z,i,density,dens[j])$pred)
      ...
      Thh.p2.p1=diag(f.help.SH2(z,i,density,rainSD,tempSD,dens[j])$pred)
      ...
      pregCat="np"
      G=h*t(outer(z,z,f.dom.gr,pregCat,i,rainSD,tempSD,density,dens[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      
      Td.np.p1=diag(f.dom.NP(z,i,density,rainSD,tempSD,dens[j])$pred)
      ...
      if(dens[j]=="1997"|dens[j]=="1998"){
        
        Td.p1.p1=diag(f.dom.F2(z,i,density,rainSD,tempSD,"1999")$pred)
      }else{ Td.p1.p1=diag(f.dom.F2(z,i,density,rainSD,tempSD,dens[j])$pred) }
      ...
      Td.p2.b=diag(f.dom.Sec(z,i,density,rainSD,tempSD,dens[j])$pred)
      ...
      Td.b.np=diag(f.dom.B(z,i,density,rainSD,tempSD,dens[j])$pred)
      
      IPM[(16*n.bins+1):(17*n.bins),(19*n.bins+1):(20*n.bins)]=G%*%Td.b.np
      
      ### after birth -> pregnant 1 month
      Td.b.p1=diag(f.dom.B2(z,i,density,rainSD,tempSD,dens[j])$pred)
      ...
      Rhelp=R
      D=h*t(outer(z,z,f.help.off.mass,i,rainSD,tempSD,density,dens[j]))
      ...
      D=h*t(outer(z,z,f.dom.off.mass,i,rainSD,tempSD,density,dens[j]))
      D=D/matrix(as.vector(apply(D,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      D[is.na(D)]=0
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.1[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  Pups.Gr.D[pi]=abs((lambdaD.1[2]-lambdaD.1[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  ### 2. Sens of pup survival to temp ###############
  lambdaD.2=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### Age 1 Pups
      ageM.pup=ageM.pup.tot[1]
      G=h*t(outer(z,z,f.pup.gr,ageM.pup,i,rainSD,tempSD,density,dens[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.pup.surv(z,ageM.pup,i,rainSD,tempSD,pert.density)$pred)
      IPM[(n.bins+1):(2*n.bins),1:n.bins]=G%*%S
    ...
    lambdaD.2[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  Pups.Surv.D[pi]=abs((lambdaD.2[2]-lambdaD.2[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  ### 3. Sens of juvenile growth to temp ################
  lambdaD.3=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=h*t(outer(z,z,f.juv.gr,ageM.juv,i,rainSD,tempSD,pert.density,dens[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.juv.surv(z,density,rainSD,tempSD)$pred)
    ...
    lambdaD.3[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  Juv.Gr.D[pi]=abs((lambdaD.3[2]-lambdaD.3[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  
  ### 4. Sens of juvenile survival to temp ##############
  lambdaD.4=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.juv.surv(z,pert.density,rainSD,tempSD)$pred)
    ...
    lambdaD.4[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  Juv.Surv.D[pi]=abs((lambdaD.4[2]-lambdaD.4[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  ### 5. Sens of subadults growth to temp ####################
  lambdaD.5=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      #### REGULAR IPM
      density=mu.cov$density[mu.cov$year==dens[j]][i]
      density=mu.cov.month$density[i] # density of the month
      ...
      G=h*t(outer(z,z,f.sub.gr,ageM.sub,i,rainSD,tempSD,pert.density,dens[j]))
      # control for eviction
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.sub.surv(z,ageM.sub,i,density,rainSD,tempSD)$pred)
    ...
    lambdaD.5[j]=get.eigen.stuff(as.matrix(IPM.full))
  ...
  Sub.Gr.D[pi]=abs((lambdaD.5[2]-lambdaD.5[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  ### 6. Sens of subadults survival to temp ###################
  lambdaD.6=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      S=diag(f.sub.surv(z,ageM.sub,i,pert.density,rainSD,tempSD)$pred)
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.6[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  Sub.Surv.D[pi]=abs((lambdaD.6[2]-lambdaD.6[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  ### 7. Sens of helper growth to temp #########################
  lambdaD.7=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### not pregnant -> staying not pregnant helper
      pregCat="np"
      G=h*t(outer(z,z,f.help.gr,pregCat,i,rainSD,tempSD,pert.density,dens[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
      Ghelp1=G
      Thh.np.p1=diag(f.help.NPH(z,i,density,rainSD,tempSD,dens[j])$pred)
    ...
    lambdaD.7[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  H.G.D[pi]=abs((lambdaD.7[2]-lambdaD.7[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  ### 8. Sens of helper survival to temp #########################
  lambdaD.8=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      S=diag(f.help.surv(z,i,pert.density,rainSD,tempSD,dens[j])$pred)
      Shelp1=S
      E=diag(f.help.emig(z,i,density,rainSD,tempSD,dens[j])$pred)
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.8[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  H.Surv.D[pi]=abs(( lambdaD.8[2]- lambdaD.8[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  ### 9. Sens of dominant growth to temp ##################
   lambdaD.9=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### non-pregnant -> non-pregnant
      pregCat="np"
      G=h*t(outer(z,z,f.dom.gr,pregCat,i,rainSD,tempSD,pert.density,dens[j]))
      G=G/matrix(as.vector(apply(G,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      G[is.na(G)]=0
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.9[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  D.G.D[pi]=abs(( lambdaD.9[2]- lambdaD.9[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  ### 10. Sens of dominant survival to temp ####################
   lambdaD.10=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      ### DOMINANT
      S=diag(f.dom.surv(z,i,pert.density,rainSD,tempSD)$pred)
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.10[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  D.Surv.D[pi]=abs(( lambdaD.10[2]- lambdaD.10[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  
  ### 11. Sens of helper recruitment to temp #######################
   lambdaD.11=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      R=diag(as.numeric(f.help.pups(z,i,density,rainSD)$pred),n.bins)
      Rhelp=R
      D=h*t(outer(z,z,f.help.off.mass,i,rainSD,tempSD,pert.density,dens[j]))
      D=D/matrix(as.vector(apply(D,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      D[is.na(D)]=0
      Dhelp=D
      IPM[1:n.bins,(15*n.bins+1):(16*n.bins)]=D%*%R
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.11[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  H.Rec.D[pi]=abs(( lambdaD.11[2]- lambdaD.11[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
  
  
  
  
  ### 12. Sens of dominant recruitment to temp #######################
   lambdaD.12=NULL
  ...
  for(j in 1:2){
    ...
    for(i in 1:12){
      ...
      # from dominant
      R=diag(f.dom.pups(z,density)$pred)
      D=h*t(outer(z,z,f.dom.off.mass,i,rainSD,tempSD,pert.density,dens[j]))
      D=D/matrix(as.vector(apply(D,2,sum)),nrow=n.bins,ncol=n.bins,byrow=TRUE)
      D[is.na(D)]=0
    ...
    IPM.full=Matrix(IPMmonth[12,,])%*%Matrix(IPMmonth[11,,])%*%Matrix(IPMmonth[10,,])%*%Matrix(IPMmonth[9,,])%*%Matrix(IPMmonth[8,,])%*%Matrix(IPMmonth[7,,])%*%Matrix(IPMmonth[6,,])%*%Matrix(IPMmonth[5,,])%*%Matrix(IPMmonth[4,,])%*%Matrix(IPMmonth[3,,])%*%Matrix(IPMmonth[2,,])%*%Matrix(IPMmonth[1,,])
     ...
     lambdaD.12[j]=get.eigen.stuff(as.matrix(IPM.full))
    
    
  }
  
  
  D.Rec.D[pi]=abs(( lambdaD.12[2]- lambdaD.12[1])/((mean(mu.cov$density[mu.cov$year==dens[2]])-mean(mu.cov$density[mu.cov$year==dens[1]]))/sd.density))
...
Results_all$lambda.sim=0
Results_all$study.length=20

write.csv(Results_all,"SensVR_Dens_Meerkats.csv",row.names = F)


library(ggplot2)
ggplot(Results_all,aes(x=vital.rates,y=sens)) + geom_boxplot()
ggplot(Results_all,aes(x=stage.age,y=sens)) + geom_boxplot()
